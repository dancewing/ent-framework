...
/**
 * Base class for all code generator implementations. This class provides many of the
 * housekeeping methods needed to implement a code generator, with only the actual code
...
public abstract class IntrospectedTable {
	...
	protected enum InternalAttribute {
		...
		ATTR_PRIMARY_KEY_TYPE, ATTR_BASE_RECORD_TYPE, ATTR_RECORD_WITH_BLOBS_TYPE, ATTR_EXAMPLE_TYPE,
		ATTR_MYBATIS3_XML_MAPPER_PACKAGE, ATTR_MYBATIS3_XML_MAPPER_FILE_NAME,
	...
	/**
	 * Returns the columns in the primary key. If the generatePrimaryKeyClass() method
	 * returns false, then these columns will be iterated as the parameters of the
	...
	/**
	 * Returns all columns in the table (for use by the select by primary key and select
	 * by example with BLOBs methods).
	...
	/**
	 * Returns all columns except BLOBs (for use by the select by example without BLOBs
	 * method).
	 * @return a List of ColumnDefinition objects for columns in the table that are non
	 * BLOBs
	 */
	 ...
	 * @return the type for the record (the class that holds non-primary key and non-BLOB
	 * fields). Note that the value will be calculated regardless of whether the table has
	 ...
	 * else use default value (targetPackage).
	 ...
	 * This list could include various types of model classes, as well as DAO classes.
	 ...
	 * This list could include a data classes, a mapper interface, extension methods, etc.
	 * @return the list of generated Kotlin files for this table
	 */
	public abstract List<GeneratedKotlinFile> getGeneratedKotlinFiles();

	/**
	 * This method should return the number of progress messages that will be send during
	...
	/**
	 * This method exists to give plugins the opportunity to replace the calculated rules
	 * if necessary.
	...
	/**
	 * Should return true if an XML generator is required for this table. This method will
	 * be called during validation of the configuration, so it should not rely on database
	 * introspection. This method simply tells the validator if an XML configuration is
	 * normally required for this implementation.
