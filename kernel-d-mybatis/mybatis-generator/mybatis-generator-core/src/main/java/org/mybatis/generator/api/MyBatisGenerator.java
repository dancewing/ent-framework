...
import org.mybatis.generator.internal.PlainTextFileMerger;
...
/**
 * This class is the main interface to MyBatis generator. A typical execution of the tool
 * involves these steps:
 *
 * <ol>
 * <li>Create a Configuration object. The Configuration can be the result of a parsing the
 * XML configuration file, or it can be created solely in Java.</li>
...
public class MyBatisGenerator {
     ...
     * @param warnings Any warnings generated during execution will be added to this list.
     * Warnings do not affect the running of the tool, but they may affect the results. A
     * typical warning is an unsupported data type. In that case, the column will be
     * ignored and generation will continue. You may specify <code>null</code> if you do
     ...
     * @param callback an instance of the ProgressCallback interface, or <code>null</code>
     * if you do not require progress information
     * @param contextIds a set of Strings containing context ids to run. Only the contexts
     * with an id specified in this list will be run. If the list is null or empty, than
     * all contexts are run.
     * @throws SQLException the SQL exception
     * @throws IOException Signals that an I/O exception has occurred.
     * @throws InterruptedException if the method is canceled through the ProgressCallback
     */
     ...
     * table name is "foo.bar". If the Set is null or empty, then all tables in the
     * configuration will be used for code generation.
     * @param writeFiles if true, then the generated files will be written to disk. If
    ...
    private void writeGeneratedJavaFile(GeneratedJavaFile gjf, ProgressCallback callback)
        ...
        String source;
        try {
            if (gjf.getWriteMode() == WriteMode.NEVER) {
                warnings.add(String.format("%s is never write to disk", gjf.getFileName()));
                return;
            }
            ...
            if (targetFile.exists()) {
                if (shellCallback.isMergeSupported()) {
                    ...
                    if (gjf.getWriteMode() == WriteMode.SKIP_ON_EXIST) {
                        warnings.add(String.format("%s is skip on exist ", targetFile.getAbsolutePath()));
                        return;
                    }
    ...
    private void writeGeneratedFile(GeneratedFile gf, ProgressCallback callback)
        ...
        try {
            ...
            if (gf.getWriteMode() == WriteMode.NEVER) {
                warnings.add(String.format("%s is never write to disk", targetFile.getAbsolutePath()));
                return;
            }
            if (targetFile.exists()) {
                if (gf.isMergeable()) {
                    source = PlainTextFileMerger.getMergedSource(gf, targetFile);
                }
                else if (shellCallback.isOverwriteEnabled()) {
                    if (gf.getWriteMode() == WriteMode.SKIP_ON_EXIST) {
                        warnings.add(String.format("%s is skip on exist ", targetFile.getAbsolutePath()));
                        return;
                    }
     ...
     * methods. This is useful if you prefer to process the generated files yourself and
     * do not want the generator to write them to disk.
    ...
    /**
     * Returns the list of generated Kotlin files after a call to one of the generate
     * methods. This is useful if you prefer to process the generated files yourself and
     * do not want the generator to write them to disk.
