...
public class DatabaseIntrospector {
	 ...
	 */
	public List<IntrospectedTable> introspectTables(TableConfiguration tc, String catalog) throws SQLException {

		// get the raw columns from the DB
		Map<ActualTableName, List<IntrospectedColumn>> columns = getColumns(tc, catalog);
	...
	private void calculateExtraColumnInformation(TableConfiguration tc,
		...
		for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns.entrySet()) {
			for (IntrospectedColumn introspectedColumn : entry.getValue()) {
				...
				FullyQualifiedJavaType fullyQualifiedJavaType = javaTypeResolver.calculateJavaType(entry.getKey(),
						introspectedColumn);
			...
			for (IntrospectedColumn introspectedColumn : entry.getValue()) {
				...
				if (columnOverride != null) {
					...
					if (stringHasValue(columnOverride.getJavaType())) {
						FullyQualifiedJavaType fqjt = new FullyQualifiedJavaType(columnOverride.getJavaType());
						if (columnOverride.getGenericTypes() != null && columnOverride.getGenericTypes().size() > 0) {
							for (String elementType : columnOverride.getGenericTypes()) {
								FullyQualifiedJavaType elementFullType = new FullyQualifiedJavaType(elementType);
								fqjt.addTypeArgument(elementFullType);
							}
						}
						introspectedColumn.setFullyQualifiedJavaType(fqjt);
					}
	...
	private Map<ActualTableName, List<IntrospectedColumn>> getColumns(TableConfiguration tc, String catalog)
			throws SQLException {
		String localCatalog;
		String localSchema;
		...
		while (rs.next()) {
			...
			// else {
			// List<IntrospectedColumn> columns = answer.computeIfAbsent(atn, k -> new
			// ArrayList<>());
			//
			// columns.add(introspectedColumn);
			// if (logger.isDebugEnabled()) {
	...
	private List<IntrospectedTable> calculateIntrospectedTables(TableConfiguration tc,
		...
		for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns.entrySet()) {
			...
			if (ContextHelper.containsSqlWildcard(tc.getTableName())) {
				// 查找是否有完全匹配的table configuration
				boolean hasExtractTableConfig = context
						.hasExtractTableConfig(introspectedTable.getFullyQualifiedTable().getIntrospectedTableName());
				if (!hasExtractTableConfig) {
					for (IntrospectedColumn introspectedColumn : entry.getValue()) {
