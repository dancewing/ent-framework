...
public class Context extends PropertyHolder {
    ...
    private final ArrayList<ColumnOverride> columnGlobals;
    ...
    public Context(ModelType defaultModelType) {
        ...
        joinConfig = new JoinConfig("", "", new HashMap<>());
    }
    ...
    public void addColumnGlobal(ColumnOverride columnGlobal) {
        columnGlobals.add(columnGlobal);
    }

    public List<ColumnOverride> getColumnGlobals() {
        return columnGlobals;
    }
     ...
     * been filled in. It does not do any more complex operations such as validating that
     ...
     * @return a list containing the results of table introspection. The list will be
     * empty if this method is called before introspectTables(), or if no tables are found
    ...
    /**
     * Introspect tables based on the configuration specified in the constructor. This
     * method is long running.
     * @param callback a progress callback if progress information is desired, or
     * <code>null</code>
     * @param warnings any warning generated from this method will be added to the List.
     * Warnings are always Strings.
     * @param fullyQualifiedTableNames a set of table names to generate. The elements of
     * the set must be Strings that exactly match what's specified in the configuration.
     * For example, if table name = "foo" and schema = "bar", then the fully qualified
     * table name is "foo.bar". If the Set is null or empty, then all tables in the
    ...
    public void introspectTables(ProgressCallback callback, List<String> warnings, Set<String> fullyQualifiedTableNames)
        ...
        try {
            ...
            //
            List<TableConfiguration> sorted = ContextHelper.mergeTableConfiguration(tableConfigurations);

            for (TableConfiguration tc : sorted) {
                ...
                List<IntrospectedTable> tables = databaseIntrospector.introspectTables(tc, connection.getCatalog());
     ...
     * configuration file. If you call this method, then you are responsible for closing
     * the connection (See {@link Context#closeConnection(Connection)}). If you do not
    ...
    public boolean hasExtractTableConfig(String tableName) {
        long result = tableConfigurations.stream()
                .filter(tableConfiguration -> StringUtils.equals(tableName, tableConfiguration.getTableName())).count();
        return result > 0;
    }
