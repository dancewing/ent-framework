...
public class ControllerPlugin extends AbstractServerPlugin {
    ...
    public boolean validate(List<String> warnings) {
        ...
        if (StringUtility.stringHasValue(enableControllerParentMode) && StringUtility.isTrue(enableControllerParentMode)) {
    ...
    public boolean clientGenerated(Interface interfaze, IntrospectedTable introspectedTable) {
        ...
        RestMethodsGenerator restMethodsGenerator = new RestMethodsGenerator(recordType, requestJavaType, responseJavaType, true);
        ...
        methodAndImports.getMethods().forEach(method -> {
            ...
            if (StringUtils.isAnyEmpty(this.responseBodyWrapper, this.responseBodySuccessStaticMethod)) {
                ...
                method.addBodyLine(String.format("return %s.%s(%s.%s(request));",
                        responseWrapJavaType.getShortName(),
        ...
        if (this.enableControllerParentMode) {
            ...
            if (this.codingStyle.equals(Constants.GENERATED_CODE_STYLE)) {
                controllerJavaClass.addAnnotation(String.format("@ApiResource(name = \"%s\")", modelClass.getDescription()));
            ...
            if (this.codingStyle.equals(Constants.GENERATED_CODE_STYLE)) {
                baseControllerJavaClass.addAnnotation(String.format("@ApiResource(name = \"%s\")", modelClass.getDescription()));
    ...
    private void addPostMapping(TopLevelClass controllerJavaClass, RestMethod method, String modelDescription) {
        if (this.codingStyle.equals(Constants.GENERATED_CODE_STYLE)) {
            method.addAnnotation(String.format("@PostResource(name = \"%s-%s\", path = \"%s\")",
    ...
    private void addGetMapping(TopLevelClass controllerJavaClass, RestMethod method, String modelDescription) {
        if (this.codingStyle.equals(Constants.GENERATED_CODE_STYLE)) {
            method.addAnnotation(String.format("@GetResource(name = \"%s-%s\", path = \"%s\")",
                    modelDescription, method.getOperation(), method.getRestPath()));
