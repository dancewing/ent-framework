...
public class WebUtils {
    ...
    public static FullyQualifiedTypescriptType convertToTypescriptImportType(String projectRootAlias, FullyQualifiedJavaType type) {
        ...
        return new FullyQualifiedTypescriptType(projectRootAlias, packageName + "." + getFileName(shortName) + "." + shortName);
    }


    public static List<Field> getFieldsWithoutPrimaryKey(List<Field> fields, String pkField) {
        return fields.stream().filter(field -> !StringUtils.equals(field.getName(), pkField)).collect(Collectors.toList());
    }
    ...
    public static List<Field> getInputFields(List<Field> fields) {
        ...
        return fields.stream().filter(field -> {
            ...
            if (beRelated.isPresent()) {
                field.setAttribute(Constants.TARGET_FIELD_RELATION, beRelated.get().getAttribute(Constants.FIELD_RELATION));
                return true;
            }
