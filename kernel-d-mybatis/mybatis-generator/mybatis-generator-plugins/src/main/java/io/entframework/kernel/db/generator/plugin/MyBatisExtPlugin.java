...
public class MyBatisExtPlugin extends AbstractDynamicSQLPlugin {
	...
	public boolean modelBaseRecordClassGenerated(TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
		...
		FullyQualifiedJavaType dynamicSqlSupportType = new FullyQualifiedJavaType(
				introspectedTable.getMyBatisDynamicSqlSupportType());
		topLevelClass
				.addAnnotation(String.format("@Table(value = \"%s\", sqlSupport = %s.class, tableProperty = \"%s\")",
						introspectedTable.getFullyQualifiedTableNameAtRuntime(), dynamicSqlSupportType.getShortName(),
	...
	private void addJoinField(TopLevelClass topLevelClass, JoinEntry joinEntry) {
		for (Pair<String, JoinTarget> detail : joinEntry.getDetails()) {
			...
			FullyQualifiedJavaType filedType = target.getType() == JoinTarget.JoinType.MORE ? listReturnType
					: recordType;
			...
			if (context.getPlugins().modelFieldGenerated(field, topLevelClass, introspectedColumn, rightTable,
				...
				if (target.getType() == JoinTarget.JoinType.MORE) {
					...
					builder.joinType(JoinTarget.JoinType.MORE).bindField(field)
					...
					field.addAnnotation(String.format("@JoinColumn(target = %s.class, left = \"%s\", right = \"%s\")",
							recordType.getShortName(), leftTableColumn.getJavaProperty(),
							rightTableColumn.getJavaProperty()));
				...
				if (target.getType() == JoinTarget.JoinType.ONE) {
					...
					builder.sourceField(relatedField).joinType(JoinTarget.JoinType.ONE).bindField(field)
					...
					field.addAnnotation(String.format("@JoinColumn(target = %s.class, left = \"%s\", right = \"%s\")",
							recordType.getShortName(), leftColumn.getJavaProperty(),
							rightTableColumn.getJavaProperty()));
