...
public class TemplateModelViewPlugin extends AbstractTemplatePlugin {
	...
	private GeneratedFile generateModelClass(TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
		...
		if (this.enableSubPackages) {
			tsBaseModelJavaType = new FullyQualifiedJavaType(
					this.targetPackage + "." + camelCaseName + "." + modelObjectName);
		}
		else {
		...
		TopLevelClass tsModelClass = (TopLevelClass) introspectedTable
				.getAttribute(Constants.INTROSPECTED_TABLE_WRAPPER_TYPESCRIPT_CLASS);
		...
		data.put("pk",
				new ModelField(GeneratorUtils.getFieldByName(tsModelClass, pkColumn.getJavaProperty()), pkColumn));
