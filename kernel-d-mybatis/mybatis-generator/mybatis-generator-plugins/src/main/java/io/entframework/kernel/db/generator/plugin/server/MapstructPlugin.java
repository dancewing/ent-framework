...
public class MapstructPlugin extends AbstractServerPlugin {
	...
	private GeneratedJavaFile generateMapstruct(TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
		...
		InnerInterface requestConverter = createInnerMapper(mapstructInterface, "RequestConverter", pojoRequestJavaType,
				recordType);
		...
		InnerInterface responseConverter = createInnerMapper(mapstructInterface, "ResponseConverter", recordType,
				pojoResponseJavaType);
	...
	private Set<FullyQualifiedJavaType> addDefaultRelation(Set<FullyQualifiedJavaType> relations,
		...
		relations.forEach(relationType -> {
			if (isRequest) {
				importTypes.addAll(createDefaultConverterMethod(innerInterface,
						getPojoRequestJavaType(relationType.getShortName()), relationType));
			}
			else {
				importTypes.addAll(createDefaultConverterMethod(innerInterface, relationType,
						getPojoResponseJavaType(relationType.getShortName())));
			}
	...
	private InnerInterface createInnerMapper(Interface mapstructInterface, String interfaceName,
		...
		mapstructInterface
				.addImportedType(new FullyQualifiedJavaType("org.mapstruct.NullValuePropertyMappingStrategy"));
