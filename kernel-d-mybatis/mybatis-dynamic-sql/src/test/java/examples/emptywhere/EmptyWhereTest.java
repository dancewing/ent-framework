...
class EmptyWhereTest {
    ...
    static List<Variation> baseVariations() {
        ...
        Variation v2 = new Variation(null, lastName, "where last_name = #{parameters.p1}");

        Variation v3 = new Variation(firstName, null, "where first_name = #{parameters.p1}");
    ...
    static Stream<Variation> joinWhereVariations() {
        ...
        baseVariations.get(
                0).whereClause = "where person.first_name = #{parameters.p1} or person.last_name = #{parameters.p2}";
    ...
    static Stream<Variation> updateWhereVariations() {
        ...
        baseVariations.get(0).whereClause = "where first_name = #{parameters.p2} or last_name = #{parameters.p3}";
    ...
    void testDeleteThreeConditions() {
        ...
        DeleteDSL<DeleteModel>.DeleteWhereBuilder builder = deleteFrom(person).where(id, isEqualTo(3));
        ...
        String expected = "delete from person" + " where id = #{parameters.p1}" + " and first_name = #{parameters.p2}"
    ...
    void testDeleteVariations(Variation variation) {
        DeleteDSL<DeleteModel>.DeleteWhereBuilder builder = deleteFrom(person).where();
    ...
    void testSelectThreeConditions() {
        ...
        QueryExpressionDSL<SelectModel>.QueryExpressionWhereBuilder builder = select(id, firstName,
                PersonDynamicSqlSupport.lastName).from(person).where(id, isEqualTo(3));
        ...
        String expected = "select id, first_name, last_name" + " from person" + " where id = #{parameters.p1}"
                + " and first_name = #{parameters.p2}" + " and last_name = #{parameters.p3}";
    ...
    void testSelectVariations(Variation variation) {
        QueryExpressionDSL<SelectModel>.QueryExpressionWhereBuilder builder = select(person.allColumns()).from(person)
                .where();
    ...
    void testJoinThreeConditions() {
        ...
        QueryExpressionDSL<SelectModel>.QueryExpressionWhereBuilder builder = select(id, firstName,
                PersonDynamicSqlSupport.lastName, orderDate).from(person).join(order)
                        .on(person.id, equalTo(order.personId)).where(id, isEqualTo(3));
        ...
        String expected = "select person.id, person.first_name, person.last_name, order.order_date" + " from person"
                + " join order on person.id = order.person_id" + " where person.id = #{parameters.p1}"
                + " and person.first_name = #{parameters.p2}" + " and person.last_name = #{parameters.p3}";
    ...
    void testJoinVariations(Variation variation) {
        QueryExpressionDSL<SelectModel>.QueryExpressionWhereBuilder builder = select(id, firstName,
                PersonDynamicSqlSupport.lastName, orderDate).from(person).join(order)
                        .on(person.id, equalTo(order.personId)).where();
        ...
        String expected = "select person.id, person.first_name, person.last_name, order.order_date" + " from person"
                + " join order on person.id = order.person_id " + variation.whereClause;
    ...
    void testUpdateThreeConditions() {
        ...
        UpdateDSL<UpdateModel>.UpdateWhereBuilder builder = update(person).set(id).equalTo(3).where(id, isEqualTo(3));
        ...
        String expected = "update person" + " set id = #{parameters.p1}" + " where id = #{parameters.p2}"
                + " and first_name = #{parameters.p3}" + " and last_name = #{parameters.p4}";
    ...
    void testUpdateVariations(Variation variation) {
        UpdateDSL<UpdateModel>.UpdateWhereBuilder builder = update(person).set(id).equalTo(3).where();
        ...
        String expected = "update person" + " set id = #{parameters.p1} " + variation.whereClause;
    ...
    void testWhereThreeConditions() {
        ...
        String expected = "where id = #{parameters.p1}" + " and first_name = #{parameters.p2}"
                + " and last_name = #{parameters.p3}";

        assertThat(whereClause.map(WhereClauseProvider::getWhereClause))
                .hasValueSatisfying(wc -> assertThat(wc).isEqualTo(expected));
    }
    ...
    void testWhereVariations(Variation variation) {
        ...
        if (variation.firstName == null && variation.lastName == null) {
            ...
            assertThat(whereClause.map(WhereClauseProvider::getWhereClause))
                    .hasValueSatisfying(wc -> assertThat(wc).isEqualTo(variation.whereClause));
        }
    }
