...
class GroupingTest {
	...
	void testSimpleGrouping() {
		SelectStatementProvider selectStatement = select(A, B, C).from(foo).where(A, isEqualTo(1), or(A, isEqualTo(2)))
				.and(B, isEqualTo(3)).build().render(RenderingStrategies.MYBATIS3);

		String expected = "select A, B, C" + " from Foo"
	...
	void testComplexGrouping() {
		SelectStatementProvider selectStatement = select(A, B, C).from(foo)
				.where(group(A, isEqualTo(1), or(A, isGreaterThan(5))), and(B, isEqualTo(1)),
						or(A, isLessThan(0), and(B, isEqualTo(2))))
				.and(C, isEqualTo(1)).build().render(RenderingStrategies.MYBATIS3);
	...
	void testGroupAndExists() {
		SelectStatementProvider selectStatement = select(A, B, C).from(foo)
				.where(group(exists(select(foo.allColumns()).from(foo).where(A, isEqualTo(3))), and(A, isEqualTo(1)),
						or(A, isGreaterThan(5))), and(B, isEqualTo(1)), or(A, isLessThan(0), and(B, isEqualTo(2))))
				.and(C, isEqualTo(1)).build().render(RenderingStrategies.MYBATIS3);
	...
	void testNestedGrouping() {
		SelectStatementProvider selectStatement = select(A, B, C).from(foo)
				.where(group(group(A, isEqualTo(1), or(A, isGreaterThan(5))), and(A, isGreaterThan(5))),
						and(group(A, isEqualTo(1), or(A, isGreaterThan(5))), or(B, isEqualTo(1))),
						or(group(A, isEqualTo(1), or(A, isGreaterThan(5))),
								and(A, isLessThan(0), and(B, isEqualTo(2)))))
				.and(C, isEqualTo(1)).build().render(RenderingStrategies.MYBATIS3);
	...
	void testAndOrCriteriaGroups() {
		SelectStatementProvider selectStatement = select(A, B, C).from(foo).where(A, isEqualTo(6)).and(C, isEqualTo(1))
				.and(group(A, isEqualTo(1), or(A, isGreaterThan(5))), or(B, isEqualTo(1)))
				.or(group(A, isEqualTo(1), or(A, isGreaterThan(5))), and(A, isLessThan(0), and(B, isEqualTo(2))))
				.build().render(RenderingStrategies.MYBATIS3);

		String expected = "select A, B, C" + " from Foo" + " where A = #{parameters.p1}" + " and C = #{parameters.p2}"
