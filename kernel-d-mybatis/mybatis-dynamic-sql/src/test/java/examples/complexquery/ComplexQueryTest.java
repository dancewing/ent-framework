...
class ComplexQueryTest {
    ...
    void testId() {
        ...
        String expected = "select person_id, first_name, last_name" + " from Person"
                + " where person_id = #{parameters.p1}" + " order by last_name, first_name"
    ...
    void testFirstNameOnly() {
        ...
        String expected = "select person_id, first_name, last_name" + " from Person"
                + " where first_name like #{parameters.p1}" + " order by last_name, first_name"
                + " fetch first #{parameters.p2} rows only";
    ...
    void testLastNameOnly() {
        ...
        String expected = "select person_id, first_name, last_name" + " from Person"
                + " where last_name like #{parameters.p1}" + " order by last_name, first_name"
                + " fetch first #{parameters.p2} rows only";
    ...
    void testBothNames() {
        ...
        String expected = "select person_id, first_name, last_name" + " from Person"
                ...
                + " order by last_name, first_name" + " fetch first #{parameters.p3} rows only";
    ...
    void testAllNull() {
        ...
        String expected = "select person_id, first_name, last_name" + " from Person" + " order by last_name, first_name"
    ...
    SelectStatementProvider search(Integer targetId, String fName, String lName) {
        QueryExpressionDSL<SelectModel>.QueryExpressionWhereBuilder builder = select(id, firstName, lastName)
                .from(person).where().configureStatement(c -> c.setNonRenderingWhereClauseAllowed(true));

        if (targetId != null) {
            builder.and(id, isEqualTo(targetId));
        }
        else {
            builder.and(firstName, isLike(fName).filter(Objects::nonNull).map(s -> "%" + s + "%")).and(lastName,
                    isLikeWhenPresent(lName).map(this::addWildcards));
        }

        builder.orderBy(lastName, firstName).fetchFirst(50).rowsOnly();
