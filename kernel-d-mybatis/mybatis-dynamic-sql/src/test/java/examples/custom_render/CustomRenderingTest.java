...
class CustomRenderingTest {
	...
	void testInsertRecord() {
		try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
			...
			InsertStatementProvider<JsonTestRecord> insertStatement = insert(record).into(jsonTest).map(id)
					.toProperty("id").map(description).toProperty("description").map(info).toProperty("info").build()
			...
			insertStatement = insert(record).into(jsonTest).map(id).toProperty("id").map(description)
					.toProperty("description").map(info).toProperty("info").build()
					.render(RenderingStrategies.MYBATIS3);

			rows = mapper.insert(insertStatement);
			assertThat(rows).isEqualTo(1);

			SelectStatementProvider selectStatement = select(id, description, info).from(jsonTest).orderBy(id).build()
					.render(RenderingStrategies.MYBATIS3);

			List<JsonTestRecord> records = mapper.selectMany(selectStatement);
			assertThat(records).hasSize(2);
			assertThat(records.get(0).getInfo())
					.isEqualTo("{\"firstName\": \"Fred\", \"lastName\": \"Flintstone\", \"age\": 30}");
			assertThat(records.get(1).getInfo())
	...
	void testGeneralInsert() {
		try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
			...
			GeneralInsertStatementProvider insertStatement = insertInto(jsonTest).set(id).toValue(1).set(description)
					.toValue("Fred").set(info)
					.toValue("{\"firstName\": \"Fred\", \"lastName\": \"Flintstone\", \"age\": 30}").build()
					.render(RenderingStrategies.MYBATIS3);
			...
			insertStatement = insertInto(jsonTest).set(id).toValue(2).set(description).toValue("Wilma").set(info)
					.toValue("{\"firstName\": \"Wilma\", \"lastName\": \"Flintstone\", \"age\": 25}").build()
					.render(RenderingStrategies.MYBATIS3);
	...
	void testInsertMultiple() {
		try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
			...
			MultiRowInsertStatementProvider<JsonTestRecord> insertStatement = insertMultiple(record1, record2)
					.into(jsonTest).map(id).toProperty("id").map(description).toProperty("description").map(info)
					.toProperty("info").build().render(RenderingStrategies.MYBATIS3);
	...
	void testUpdate() {
		try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
			...
			UpdateStatementProvider updateStatement = update(jsonTest).set(info)
					.equalTo("{\"firstName\": \"Wilma\", \"lastName\": \"Flintstone\", \"age\": 26}")
					.where(id, isEqualTo(2)).build().render(RenderingStrategies.MYBATIS3);

			String expected = "update JsonTest " + "set info = #{parameters.p1,jdbcType=VARCHAR}::json "
	...
	void testDeReference() {
		try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(id, description, info).from(jsonTest)
					.where(dereference(info, "age"), isEqualTo("25")).build().render(RenderingStrategies.MYBATIS3);

			String expected = "select id, description, info " + "from JsonTest "
			...
			assertThat(record).hasValueSatisfying(r -> assertThat(r.getInfo())
					.isEqualTo("{\"firstName\": \"Wilma\", \"lastName\": \"Flintstone\", \"age\": 25}"));
		}
	}
	...
	void testDereference2() {
		try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(dereference(info, "firstName").as("firstname"))
					.from(jsonTest).where(dereference(info, "age"), isEqualTo("25")).build()
					.render(RenderingStrategies.MYBATIS3);

			String expected = "select info->>'firstName' as firstname " + "from JsonTest "
					+ "where info->>'age' = #{parameters.p1,jdbcType=VARCHAR}";
