...
class GroupByTest {
	...
	void testBasicGroupBy() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(gender, count()).from(person).groupBy(gender).build()
	...
	void testBasicGroupByWithAggregateAlias() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(gender, count().as("count")).from(person).groupBy(gender)
	...
	void testGroupByAfterJoin() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(lastName, streetAddress, count().as("count"))
					...
					.groupBy(lastName, streetAddress).build().render(RenderingStrategies.MYBATIS3);
	...
	void testUnionAfterJoin() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(lastName, firstName, streetAddress).from(person, "p")
					.join(address, "a").on(person.addressId, equalTo(address.id)).union()
					.select(person2.lastName, person2.firstName, streetAddress).from(person2, "p").join(address, "a")
					.on(person2.addressId, equalTo(address.id)).orderBy(lastName, firstName).build()
					.render(RenderingStrategies.MYBATIS3);

			String expected = "select p.last_name, p.first_name, a.street_address"
					+ " from Person p join Address a on p.address_id = a.address_id" + " union"
	...
	void testUnionAllAfterJoin() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(lastName, firstName, streetAddress).from(person, "p")
					.join(address, "a").on(person.addressId, equalTo(address.id)).unionAll()
					.select(person2.lastName, person2.firstName, streetAddress).from(person2, "p").join(address, "a")
					.on(person2.addressId, equalTo(address.id)).orderBy(lastName, firstName).build()
					.render(RenderingStrategies.MYBATIS3);

			String expected = "select p.last_name, p.first_name, a.street_address"
					+ " from Person p join Address a on p.address_id = a.address_id" + " union all"
					+ " select p.last_name, p.first_name, a.street_address"
					+ " from Person2 p join Address a on p.address_id = a.address_id"
					+ " order by last_name, first_name";
			assertThat(selectStatement.getSelectStatement()).isEqualTo(expected);
	...
	void testUnionAfterGroupBy() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(stringConstant("Gender"), gender.as("value"),
					count().as("count")).from(person).groupBy(gender).union()
							.select(stringConstant("Last Name"), lastName.as("value"), count().as("count")).from(person)
							.groupBy(lastName).build().render(RenderingStrategies.MYBATIS3);
	...
	void testUnionAllAfterGroupBy() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(stringConstant("Gender"), gender.as("value"),
					count().as("count")).from(person).groupBy(gender).unionAll()
							.select(stringConstant("Last Name"), lastName.as("value"), count().as("count")).from(person)
							.groupBy(lastName).build().render(RenderingStrategies.MYBATIS3);
	...
	void testBasicGroupByOrderByWithAggregateAlias() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(gender, count().as("count")).from(person).groupBy(gender)
					.orderBy(gender).build().render(RenderingStrategies.MYBATIS3);
	...
	void testBasicGroupByOrderByWithCalculatedColumnAndTableAlias() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(substring(gender, 1, 1).as("ShortGender"),
					avg(age).as("AverageAge")).from(person, "a").groupBy(substring(gender, 1, 1))
							.orderBy(sortColumn("ShortGender").descending()).build()
							.render(RenderingStrategies.MYBATIS3);
	...
	void testGroupByAfterWhere() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(lastName, count().as("count")).from(person, "a")
					.where(gender, isEqualTo("Male")).groupBy(lastName).build().render(RenderingStrategies.MYBATIS3);
	...
	void testLimitAndOffsetAfterGroupBy() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(lastName, count().as("count")).from(person)
					.groupBy(lastName).limit(1).offset(1).build().render(RenderingStrategies.MYBATIS3);
	...
	void testLimitOnlyAfterGroupBy() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(lastName, count().as("count")).from(person)
					.groupBy(lastName).limit(1).build().render(RenderingStrategies.MYBATIS3);
	...
	void testOffsetOnlyAfterGroupBy() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(lastName, count().as("count")).from(person)
					.groupBy(lastName).offset(1).build().render(RenderingStrategies.MYBATIS3);
	...
	void testOffsetAndFetchFirstAfterGroupBy() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(lastName, count().as("count")).from(person)
					.groupBy(lastName).offset(1).fetchFirst(1).rowsOnly().build().render(RenderingStrategies.MYBATIS3);
	...
	void testFetchFirstOnlyAfterGroupBy() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(lastName, count().as("count")).from(person)
					.groupBy(lastName).fetchFirst(1).rowsOnly().build().render(RenderingStrategies.MYBATIS3);
	...
	void testCountDistinct() {
		try (SqlSession session = sqlSessionFactory.openSession()) {
			...
			SelectStatementProvider selectStatement = select(countDistinct(lastName).as("count")).from(person).build()
					.render(RenderingStrategies.MYBATIS3);
