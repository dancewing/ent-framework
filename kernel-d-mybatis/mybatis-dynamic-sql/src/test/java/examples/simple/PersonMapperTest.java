...
class PersonMapperTest {
    ...
    void testSelectEmployed() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            List<PersonRecord> rows = mapper.select(c -> c.where(employed, isTrue()).orderBy(id));
    ...
    void testSelectUnemployed() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            List<PersonRecord> rows = mapper.select(c -> c.where(employed, isFalse()).orderBy(id));
    ...
    void testSelectWithTypeConversion() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            List<PersonRecord> rows = mapper
                    .select(c -> c.where(id, isEqualTo("1").map(Integer::parseInt)).or(occupation, isNull()));
    ...
    void testSelectWithTypeConversionAndFilterAndNull() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            List<PersonRecord> rows = mapper
                    .select(c -> c.where(id, isEqualTo((String) null).filter(Objects::nonNull).map(Integer::parseInt))
    ...
    void testGeneralSelect() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = select(id.as("A_ID"), firstName, lastName, birthDate, employed,
                    occupation, addressId).from(person).where(id, isEqualTo(1)).or(occupation, isNull()).build()
    ...
    void testSelectDistinct() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            List<PersonRecord> rows = mapper
                    .selectDistinct(c -> c.where(id, isGreaterThan(1)).or(occupation, isNull()));
    ...
    void testSelectWithTypeHandler() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            List<PersonRecord> rows = mapper.select(c -> c.where(employed, isEqualTo(false)).orderBy(id));

            assertAll(() -> assertThat(rows).hasSize(2), () -> assertThat(rows.get(0).getId()).isEqualTo(3),
    ...
    void testFirstNameIn() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            List<PersonRecord> rows = mapper.select(c -> c.where(firstName, isIn("Fred", "Barney")));

            assertAll(() -> assertThat(rows).hasSize(2),
    ...
    void testOrderByCollection() {
        ...
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            List<PersonRecord> rows = mapper
                    .select(c -> c.where(firstName, isIn("Fred", "Barney")).orderBy(orderByColumns));
    ...
    void testGeneralInsert() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            PersonMapper mapper = session.getMapper(PersonMapper.class);
            int rows = mapper.generalInsert(c -> c.set(id).toValue(100).set(firstName).toValue("Joe").set(lastName)
                    .toValue(LastName.of("Jones")).set(birthDate).toValue(new Date()).set(employed).toValue(true)
                    .set(occupation).toValue("Developer").set(addressId).toValue(1));
    ...
    void testUpdate() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            rows = mapper.update(c -> PersonMapper.updateAllColumns(record, c).where(id, isEqualTo(100)).and(firstName,
                    isEqualTo("Joe")));
    ...
    void testUpdateOneField() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            rows = mapper.update(c -> c.set(occupation).equalTo("Programmer").where(id, isEqualTo(100)));
    ...
    void testUpdateAll() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            PersonRecord updateRecord = new PersonRecord();
            updateRecord.setOccupation("Programmer");
            rows = mapper.update(c -> PersonMapper.updateSelectiveColumns(updateRecord, c));
    ...
    void testUpdateSelective() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            PersonRecord updateRecord = new PersonRecord();
            updateRecord.setOccupation("Programmer");
            rows = mapper.update(c -> PersonMapper.updateSelectiveColumns(updateRecord, c).where(id, isEqualTo(100)));
    ...
    void testTypeHandledLike() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            List<PersonRecord> rows = mapper.select(c -> c.where(lastName, isLike(LastName.of("Fl%"))).orderBy(id));
    ...
    void testTypeHandledNotLike() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            List<PersonRecord> rows = mapper.select(c -> c.where(lastName, isNotLike(LastName.of("Fl%"))).orderBy(id));
    ...
    void testJoinAllRows() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            List<PersonWithAddress> records = mapper.select(SelectDSLCompleter.allRowsOrderedBy(id));
    ...
    void testWithEnumOrdinalTypeHandler() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            GeneralInsertStatementProvider insertStatement = insertInto(address).set(address.id).toValue(4)
                    .set(address.streetAddress).toValue("987 Elm Street").set(address.city).toValue("Mayberry")
                    .set(address.state).toValue("NC").set(address.addressType).toValue(AddressRecord.AddressType.HOME)
                    .build().render(RenderingStrategies.MYBATIS3);

            int rows = mapper.generalInsert(insertStatement);
            assertThat(rows).isEqualTo(1);

            SelectStatementProvider selectStatement = select(address.addressType).from(address)
                    .where(address.id, isEqualTo(4)).build().render(RenderingStrategies.MYBATIS3);
