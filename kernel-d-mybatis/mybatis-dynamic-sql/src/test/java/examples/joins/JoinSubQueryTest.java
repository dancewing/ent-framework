...
class JoinSubQueryTest {
        ...
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = select(orderMaster.orderId, orderMaster.orderDate,
                    orderDetail.lineNumber, orderDetail.description, orderDetail.quantity)
                            .from(orderMaster, "om")
                            .join(select(orderDetail.orderId, orderDetail.lineNumber, orderDetail.description,
                                    orderDetail.quantity).from(orderDetail), "od")
                            .on(orderMaster.orderId, equalTo(orderDetail.orderId.qualifiedWith("od"))).build()
        ...
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = select(orderMaster.orderId, orderMaster.orderDate,
                    orderLine.lineNumber, itemMaster.description, orderLine.quantity)
                            ...
                            .join(select(itemMaster.itemId, itemMaster.description).from(itemMaster), "im")
                            .on(orderLine.itemId.qualifiedWith("ol"), equalTo(itemMaster.itemId.qualifiedWith("im")))
                            .where(orderMaster.orderId, isEqualTo(2)).build().render(RenderingStrategies.MYBATIS3);
    ...
    void testMultipleTableJoinWithSelectStar() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = select(orderMaster.orderId, orderMaster.orderDate,
                    orderLine.lineNumber, itemMaster.description, orderLine.quantity).from(orderMaster, "om")
                            .join(select(orderLine.allColumns()).from(orderLine), "ol")
                            .on(orderMaster.orderId, equalTo(orderLine.orderId.qualifiedWith("ol")))
                            .join(select(itemMaster.allColumns()).from(itemMaster), "im")
                            .on(orderLine.itemId.qualifiedWith("ol"), equalTo(itemMaster.itemId.qualifiedWith("im")))
                            .where(orderMaster.orderId, isEqualTo(2)).orderBy(orderMaster.orderId).build()
                            .render(RenderingStrategies.MYBATIS3);
            ...
            String expectedStatement = "select om.order_id, om.order_date, line_number, description, quantity"
                    ...
                    + " where om.order_id = #{parameters.p1,jdbcType=INTEGER}" + " order by order_id";
            assertThat(selectStatement.getSelectStatement()).isEqualTo(expectedStatement);
    ...
    void testRightJoin() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = select(orderLine.orderId, orderLine.quantity,
                    itemMaster.itemId.qualifiedWith("im"), itemMaster.description).from(orderLine, "ol")
    ...
    void testRightJoin2() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = select(orderLine.orderId, orderLine.quantity,
                    itemMaster.itemId.qualifiedWith(("im")), itemMaster.description).from(orderMaster, "om")
    ...
    void testLeftJoin() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = select(orderLine.orderId, orderLine.quantity,
                    itemMaster.itemId.qualifiedWith("im"), itemMaster.description).from(itemMaster, "im")
                            .leftJoin(select(orderLine.allColumns()).from(orderLine), "ol")
                            .on(orderLine.itemId.qualifiedWith("ol"), equalTo(itemMaster.itemId))
                            .orderBy(itemMaster.itemId).build().render(RenderingStrategies.MYBATIS3);

            String expectedStatement = "select order_id, quantity, im.item_id, im.description" + " from ItemMaster im"
                    + " left join (select * from OrderLine) ol on ol.item_id = im.item_id" + " order by item_id";
            assertThat(selectStatement.getSelectStatement()).isEqualTo(expectedStatement);
    ...
    void testFullJoin() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = select(orderLine.orderId, orderLine.quantity,
                    orderLine.itemId.as("ol_itemid").qualifiedWith("ol"), itemMaster.itemId.as("im_itemid"),
                    itemMaster.description).from(itemMaster, "im")
            ...
            String expectedStatement = "select order_id, quantity, ol.item_id as ol_itemid, im.item_id as im_itemid, im.description"
                    + " from ItemMaster im" + " full join (select * from OrderLine) ol on im.item_id = ol.item_id"
