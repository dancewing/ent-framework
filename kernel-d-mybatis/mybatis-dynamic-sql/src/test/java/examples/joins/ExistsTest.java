...
class ExistsTest {
    ...
    void testExists() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = select(itemMaster.allColumns()).from(itemMaster, "im")
                    .where(exists(select(orderLine.allColumns()).from(orderLine, "ol").where(orderLine.itemId,
                            isEqualTo(itemMaster.itemId.qualifiedWith("im")))))
                    .orderBy(itemMaster.itemId).build().render(RenderingStrategies.MYBATIS3);
    ...
    void testNotExists() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = select(itemMaster.allColumns()).from(itemMaster, "im")
                    .where(notExists(select(orderLine.allColumns()).from(orderLine, "ol").where(orderLine.itemId,
                            isEqualTo(itemMaster.itemId.qualifiedWith("im")))))
                    .orderBy(itemMaster.itemId).build().render(RenderingStrategies.MYBATIS3);
    ...
    void testNotExistsWithNotCriterion() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = select(itemMaster.allColumns()).from(itemMaster, "im")
                    .where(not(exists(select(orderLine.allColumns()).from(orderLine, "ol").where(orderLine.itemId,
                            isEqualTo(itemMaster.itemId.qualifiedWith("im"))))))
                    .orderBy(itemMaster.itemId).build().render(RenderingStrategies.MYBATIS3);
    ...
    void testAndExists() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = select(itemMaster.allColumns()).from(itemMaster, "im")
                    ...
                    .and(exists(select(orderLine.allColumns()).from(orderLine, "ol").where(orderLine.itemId,
                            isEqualTo(itemMaster.itemId.qualifiedWith("im")))))
                    .orderBy(itemMaster.itemId).build().render(RenderingStrategies.MYBATIS3);

            String expectedStatement = "select im.* from ItemMaster im"
                    + " where im.item_id = #{parameters.p1,jdbcType=INTEGER}"
                    + " and exists (select ol.* from OrderLine ol where ol.item_id = im.item_id)" + " order by item_id";
            assertThat(selectStatement.getSelectStatement()).isEqualTo(expectedStatement);
    ...
    void testAndExistsAnd() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            String expectedStatement = "select im.* from ItemMaster im"
                    ...
                    + " and im.item_id > #{parameters.p2,jdbcType=INTEGER})" + " order by item_id";
            assertThat(selectStatement.getSelectStatement()).isEqualTo(expectedStatement);
    ...
    void testOrExists() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = select(itemMaster.allColumns()).from(itemMaster, "im")
                    .where(itemMaster.itemId, isEqualTo(22))
                    .or(exists(select(orderLine.allColumns()).from(orderLine, "ol").where(orderLine.itemId,
                            isEqualTo(itemMaster.itemId.qualifiedWith("im")))))
                    .orderBy(itemMaster.itemId).build().render(RenderingStrategies.MYBATIS3);

            String expectedStatement = "select im.* from ItemMaster im"
                    + " where im.item_id = #{parameters.p1,jdbcType=INTEGER}"
                    + " or exists (select ol.* from OrderLine ol where ol.item_id = im.item_id)" + " order by item_id";
            assertThat(selectStatement.getSelectStatement()).isEqualTo(expectedStatement);
    ...
    void testWhereExistsOr() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = select(itemMaster.allColumns()).from(itemMaster, "im")
                    .where(exists(select(orderLine.allColumns()).from(orderLine, "ol").where(orderLine.itemId,
                            isEqualTo(itemMaster.itemId.qualifiedWith("im")))), or(itemMaster.itemId, isEqualTo(22)))
                    .orderBy(itemMaster.itemId).build().render(RenderingStrategies.MYBATIS3);

            String expectedStatement = "select im.* from ItemMaster im"
                    + " where (exists (select ol.* from OrderLine ol where ol.item_id = im.item_id)"
                    + " or im.item_id = #{parameters.p1,jdbcType=INTEGER})" + " order by item_id";
            assertThat(selectStatement.getSelectStatement()).isEqualTo(expectedStatement);
    ...
    void testWhereExistsAnd() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = select(itemMaster.allColumns()).from(itemMaster, "im")
                    .where(exists(select(orderLine.allColumns()).from(orderLine, "ol").where(orderLine.itemId,
                            isEqualTo(itemMaster.itemId.qualifiedWith("im")))), and(itemMaster.itemId, isEqualTo(22)))
                    .orderBy(itemMaster.itemId).build().render(RenderingStrategies.MYBATIS3);

            String expectedStatement = "select im.* from ItemMaster im"
                    + " where (exists (select ol.* from OrderLine ol where ol.item_id = im.item_id)"
                    + " and im.item_id = #{parameters.p1,jdbcType=INTEGER})" + " order by item_id";
            assertThat(selectStatement.getSelectStatement()).isEqualTo(expectedStatement);
    ...
    void testDeleteWithHardAlias() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            DeleteStatementProvider deleteStatement = deleteFrom(im).where(notExists(
                    select(orderLine.allColumns()).from(orderLine, "ol").where(orderLine.itemId, isEqualTo(im.itemId))))
    ...
    void testUpdateWithHardAlias() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            UpdateStatementProvider updateStatement = update(im)
                    .set(im.description).equalTo("No Orders").where(notExists(select(orderLine.allColumns())
                            .from(orderLine, "ol").where(orderLine.itemId, isEqualTo(im.itemId))))
                    .build().render(RenderingStrategies.MYBATIS3);
    ...
    void testUpdateWithSoftAlias() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            UpdateStatementProvider updateStatement = update(itemMaster, "im").set(itemMaster.description)
                    .equalTo("No Orders").where(notExists(select(orderLine.allColumns()).from(orderLine, "ol")
