...
class NoInitialConditionTest {
	...
	void testNoInitialConditionSingleSub() {
		...
		String expected = "select column1, column2 from foo where column1 < :p1 " + "and column2 = :p2";
	...
	void testNoInitialConditionWhereMultipleSubs() {
		...
		SelectStatementProvider selectStatement = select(column1, column2).from(foo).where(criteria).build()
	...
	void testNoInitialConditionWhereNotMultipleSubs() {
		...
		SelectStatementProvider selectStatement = select(column1, column2).from(foo)
				.where(not(criteria), and(column1, isLessThan(new Date()))).build()
				.render(RenderingStrategies.SPRING_NAMED_PARAMETER);

		String expected = "select column1, column2 from foo where (not "
				+ "(column2 = :p1 or column2 = :p2 or column2 = :p3) " + "and column1 < :p4)";
	...
	void testNoInitialConditionWhereGroupMultipleSubs() {
		...
		SelectStatementProvider selectStatement = select(column1, column2).from(foo)
				.where(group(criteria), and(column1, isLessThan(new Date()))).build()
				.render(RenderingStrategies.SPRING_NAMED_PARAMETER);
	...
	void testNoInitialConditionWhereCCAndMultipleSubs() {
		...
		SelectStatementProvider selectStatement = select(column1, column2).from(foo)
				.where(column1, isLessThan(new Date()), and(criteria)).build()
				.render(RenderingStrategies.SPRING_NAMED_PARAMETER);
	...
	void testNoInitialConditionWhereCCOrMultipleSubs() {
		...
		SelectStatementProvider selectStatement = select(column1, column2).from(foo)
				.where(column1, isLessThan(new Date()), or(criteria)).build()
				.render(RenderingStrategies.SPRING_NAMED_PARAMETER);
	...
	private SelectStatementProvider buildSelectStatement(List<AndOrCriteriaGroup> criteria) {
		return select(column1, column2).from(foo).where(column1, isLessThan(new Date())).and(criteria).build()
				.render(RenderingStrategies.SPRING_NAMED_PARAMETER);
	}
