...
class GeneralInsertStatementTest {
	...
	void testFullInsertStatementBuilder() {
		...
		GeneralInsertStatementProvider insertStatement = insertInto(foo).set(id).toValue(2).set(firstName)
				.toValue("Jones").set(occupation).toValue("dino driver").build().render(RenderingStrategies.MYBATIS3);

		String expectedStatement = "insert into foo " + "(id, first_name, occupation) "
	...
	void testInsertStatementBuilderWithNulls() {
		...
		GeneralInsertStatementProvider insertStatement = insertInto(foo).set(id).toValue(1).set(firstName)
				.toValue("Fred").set(lastName).toValue("Smith").set(occupation).toNull().build()
	...
	void testInsertStatementBuilderWithConstants() {
		...
		GeneralInsertStatementProvider insertStatement = insertInto(foo).set(id).toConstant("3").set(firstName)
				.toValue("Fred").set(lastName).toValue("Jones").set(occupation).toStringConstant("Y").build()
				.render(RenderingStrategies.MYBATIS3);
	...
	void testSelectiveInsertStatementBuilder() {
		...
		GeneralInsertStatementProvider insertStatement = insertInto(foo).set(id).toValueWhenPresent(() -> myId)
				.set(firstName).toValueWhenPresent(myFirstName).set(lastName).toValueWhenPresent(() -> myLastName)
				.set(occupation).toValueWhenPresent(myOccupation).build().render(RenderingStrategies.MYBATIS3);
