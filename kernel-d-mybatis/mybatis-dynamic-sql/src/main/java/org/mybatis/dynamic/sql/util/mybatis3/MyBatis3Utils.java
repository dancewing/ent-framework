...
public class MyBatis3Utils {
	...
	public static SelectStatementProvider countFrom(CountDSL<SelectModel> start, CountDSLCompleter completer) {
		return completer.apply(start).build().render(RenderingStrategies.MYBATIS3);
	}

	public static long countFrom(ToLongFunction<SelectStatementProvider> mapper, CountDSL<SelectModel> start,
			CountDSLCompleter completer) {
	...
	public static DeleteStatementProvider deleteFrom(SqlTable table, DeleteDSLCompleter completer) {
		return completer.apply(SqlBuilder.deleteFrom(table)).build().render(RenderingStrategies.MYBATIS3);
	}

	public static DeleteStatementProvider deleteFrom(Class<?> entityClass, DeleteDSLCompleter completer) {
		return completer.apply(SqlBuilder.deleteFrom(entityClass)).build().render(RenderingStrategies.MYBATIS3);
	}
	...
	public static int deleteFrom(ToIntFunction<DeleteStatementProvider> mapper, Class<?> entityClass,
			DeleteDSLCompleter completer) {
	...
	public static <R> InsertStatementProvider<R> insert(R row, SqlTable table, UnaryOperator<InsertDSL<R>> completer) {
		return completer.apply(SqlBuilder.insert(row).into(table)).build().render(RenderingStrategies.MYBATIS3);
	}
	...
	public static GeneralInsertStatementProvider generalInsert(SqlTable table,
		...
		return completer.apply(GeneralInsertDSL.insertInto(table)).build().render(RenderingStrategies.MYBATIS3);
	}

	public static int generalInsert(ToIntFunction<GeneralInsertStatementProvider> mapper, SqlTable table,
			UnaryOperator<GeneralInsertDSL> completer) {
	...
	public static <R> MultiRowInsertStatementProvider<R> insertMultiple(Collection<R> records, SqlTable table,
		...
		return completer.apply(SqlBuilder.insertMultiple(records).into(table)).build()
	...
	public static <R> List<R> selectList(Function<SelectStatementProvider, List<R>> mapper, BasicColumn[] selectList,
			SqlTable table, SelectDSLCompleter completer) {
		return mapper.apply(select(selectList, table, completer));
	}

	public static <R> List<R> selectList(Function<SelectStatementProvider, List<R>> mapper, Class<?> entityClass,
			SelectDSLCompleter completer) {
	...
	public static <R> R selectOne(Function<SelectStatementProvider, R> mapper, BasicColumn[] selectList, SqlTable table,
			SelectDSLCompleter completer) {
		return mapper.apply(select(selectList, table, completer));
	}

	public static <R> R selectOne(Function<SelectStatementProvider, R> mapper, Class<?> entityClass,
			SelectDSLCompleter completer) {
		EntityMeta entities = Entities.getInstance(entityClass);
		return mapper.apply(select(entities.getSelectColumns(), entityClass, completer));
	}

	public static <R> R selectOne(Function<SelectStatementProvider, R> mapper, QueryExpressionDSL<SelectModel> start,
			SelectDSLCompleter completer) {
		return mapper.apply(select(start, completer));
	}

	public static UpdateStatementProvider update(SqlTable table, UpdateDSLCompleter completer) {
		return completer.apply(SqlBuilder.update(table)).build().render(RenderingStrategies.MYBATIS3);
	}

	public static UpdateStatementProvider update(Class<?> entityClass, UpdateDSLCompleter completer) {
		return completer.apply(SqlBuilder.update(entityClass)).build().render(RenderingStrategies.MYBATIS3);
	}

	public static int update(ToIntFunction<UpdateStatementProvider> mapper, SqlTable table,
			UpdateDSLCompleter completer) {
		return mapper.applyAsInt(update(table, completer));
	}

	public static int update(ToIntFunction<UpdateStatementProvider> mapper, Class<?> entityClass,
			UpdateDSLCompleter completer) {
		return mapper.applyAsInt(update(entityClass, completer));
	}

	public static <R> UpdateRowStatementProvider<R> updateRow(R row, SqlTable table,
			UnaryOperator<UpdateRowDSL<R>> completer) {
		return completer.apply(SqlBuilder.updateRow(row).into(table)).build().render(RenderingStrategies.MYBATIS3);
	}

	public static <R> int updateRow(ToIntFunction<UpdateRowStatementProvider<R>> mapper, R row, SqlTable table,
			UnaryOperator<UpdateRowDSL<R>> completer) {
