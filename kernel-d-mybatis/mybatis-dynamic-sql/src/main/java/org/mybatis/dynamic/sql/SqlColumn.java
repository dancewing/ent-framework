...
public class SqlColumn<T> implements BindableColumn<T>, SortSpecification {
    ...
    /**
     * Override the calculated table qualifier if there is one. This is useful for
     * sub-queries where the calculated table qualifier may not be correct in all cases.
    ...
    /**
     * Set an alias with a camel cased string based on the column name. The can be useful
     * for queries using the
     * {@link org.mybatis.dynamic.sql.util.mybatis3.CommonSelectMapper} where the columns
     * are placed into a map based on the column name returned from the database.
     *
     * <p>
     * A camel case string is mixed case, and most databases do not support unquoted mixed
     * case strings as identifiers. Therefore the generated alias will be surrounded by
     * double quotes thereby making it a quoted identifier. Most databases will respect
    ...
    public String renderWithTableAlias(TableAliasCalculator tableAliasCalculator) {
        return tableQualifierFunction.apply(tableAliasCalculator, table).map(this::applyTableAlias)
                .orElseGet(this::name);
    }
    ...
    /**
     * This method helps us tell a bit of fiction to the Java compiler. Java, for better
     * or worse, does not carry generic type information through chained methods. We want
     * to enable method chaining in the "with" methods. With this bit of fiction, we force
     * the compiler to delay type inference to the last method in the chain.
    ...
    private <S> Builder<S> copy() {
        return new Builder<S>().withName(this.name).withTable(this.table).withJdbcType(this.jdbcType)
                .withDescending(this.isDescending).withAlias(this.alias).withTypeHandler(this.typeHandler)
                ...
                .withTableQualifierFunction(this.tableQualifierFunction).withJavaType((Class<S>) this.javaType);
    }
    ...
    public static <T> SqlColumn<T> of(String name, SqlTable table) {
        return new Builder<T>().withName(name).withTable(table).build();
    }

    public static <T> SqlColumn<T> of(String name, SqlTable table, JDBCType jdbcType) {
        return new Builder<T>().withName(name).withTable(table).withJdbcType(jdbcType).build();
    }
