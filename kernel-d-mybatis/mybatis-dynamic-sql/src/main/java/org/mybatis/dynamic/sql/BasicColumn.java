...
/**
 * Describes attributes of columns that are necessary for rendering if the column is not
 * expected to be bound as a JDBC parameter. Columns in select lists, join expressions,
 * and group by expressions are typically not bound.
 *
 * @author Jeff Butler
 */
public interface BasicColumn {
	...
	/**
	 * Returns the name of the item aliased with a table name if appropriate. For example,
	 * "a.foo". This is appropriate for where clauses and order by clauses.
	...
	/**
	 * Returns the name of the item aliased with a table name and column alias if
	 * appropriate. For example, "a.foo as bar". This is appropriate for select list
	 * clauses.
	 * @param tableAliasCalculator the table alias calculator for the current renderer
