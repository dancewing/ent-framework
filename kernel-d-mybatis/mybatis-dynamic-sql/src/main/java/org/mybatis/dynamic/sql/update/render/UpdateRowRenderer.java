...
public class UpdateRowRenderer<T> {
    ...
    public UpdateRowStatementProvider<T> render() {
        ...
        List<Optional<SetAndWhere>> fieldsAndValues = model.mapColumnMappings(m -> m.accept(visitor)).toList();
        ...
        List<Optional<SetAndWhere>> whereCondition = model.whereMappings(m -> m.accept(visitor)).toList();

        if (whereCondition.stream().noneMatch(Optional::isPresent)) {
            throw new InvalidSqlException(Messages.getString("ERROR.33")); //$NON-NLS-1$
        }

        List<Optional<SetAndWhere>> parameters = model.parameterMappings(m -> m.accept(visitor)).toList();
        ...
        return DefaultUpdateRowStatementProvider.withRow(model.row())
                ...
                .withParameters(calculateParameters(parameters)).build();
    }

    private Map<String, Object> calculateParameters(List<Optional<SetAndWhere>> parameters) {
        Map<String, Object> results = new HashMap<>();
        if (parameters != null) {
            parameters.stream().filter(Optional::isPresent).map(Optional::get).forEach(setAndWhere -> {
    ...
    private String calculateUpdateStatement(List<Optional<SetAndWhere>> fieldsAndValues,
            List<Optional<SetAndWhere>> whereCondition) {
        return "update" //$NON-NLS-1$
                + spaceBefore(model.table().tableNameAtRuntime()) + spaceBefore("set")
                + spaceBefore(calculateSetFieldAndValuePhrase(fieldsAndValues)) + spaceBefore("where")
