...
public class QueryExpressionDSL<R>
    ...
    QueryExpressionDSL(FromGatherer<R> fromGatherer, TableExpression table, Class<?> entityClass) {
        super(table);
        connector = fromGatherer.connector;
        selectList = fromGatherer.selectList;
        isDistinct = fromGatherer.isDistinct;
    ...
    protected QueryExpressionModel buildModel() {
        QueryExpressionModel.Builder builder = QueryExpressionModel.withSelectList(selectList).withConnector(connector)
                .withTable(table()).withEntityClass(entityClass).isDistinct(isDistinct).withTableAliases(tableAliases())
                .withJoinModel(buildJoinModel().orElse(null)).withGroupByModel(groupByModel);
    ...
    public static class FromGatherer<R> {
        ...
        public QueryExpressionDSL<R> from(Buildable<SelectModel> select, String tableAlias, Class<?> entityClass) {
            return selectDSL.newQueryExpression(this, buildSubQuery(select, tableAlias), entityClass);
        }
        ...
        public QueryExpressionDSL<R> from(Class<?> entityClass) {
            return selectDSL.newQueryExpression(this, entityClass);
        }
    ...
    public class JoinSpecificationFinisher extends
            AbstractWhereSupport<QueryExpressionWhereBuilder, JoinSpecificationFinisher> implements Buildable<R> {

        private final JoinSpecification.Builder joinSpecificationBuilder;

        public JoinSpecificationFinisher(TableExpression table, BasicColumn joinColumn, JoinCondition joinCondition,
                JoinType joinType) {
            JoinCriterion joinCriterion = new JoinCriterion.Builder().withConnector("on") //$NON-NLS-1$
                    .withJoinColumn(joinColumn).withJoinCondition(joinCondition).build();

            joinSpecificationBuilder = JoinSpecification.withJoinTable(table).withJoinType(joinType)
        ...
        public JoinSpecificationFinisher(TableExpression table, BasicColumn joinColumn, JoinCondition joinCondition,
                JoinType joinType, JoinCriterion... andJoinCriteria) {
            JoinCriterion onJoinCriterion = new JoinCriterion.Builder().withConnector("on") //$NON-NLS-1$
                    .withJoinColumn(joinColumn).withJoinCondition(joinCondition).build();

            joinSpecificationBuilder = JoinSpecification.withJoinTable(table).withJoinType(joinType)
                    .withJoinCriterion(onJoinCriterion).withJoinCriteria(Arrays.asList(andJoinCriteria));
        ...
        public JoinSpecificationFinisher and(BasicColumn joinColumn, JoinCondition joinCondition) {
            JoinCriterion joinCriterion = new JoinCriterion.Builder().withConnector("and") //$NON-NLS-1$
                    .withJoinColumn(joinColumn).withJoinCondition(joinCondition).build();
    ...
    public class UnionBuilder {
        ...
        public FromGatherer<R> select(List<BasicColumn> selectList) {
            return new FromGatherer.Builder<R>().withConnector(connector).withSelectList(selectList)
                    .withSelectDSL(selectDSL).build();
        }
        ...
        public FromGatherer<R> selectDistinct(List<BasicColumn> selectList) {
            return new FromGatherer.Builder<R>().withConnector(connector).withSelectList(selectList)
                    .withSelectDSL(selectDSL).isDistinct().build();
        }
