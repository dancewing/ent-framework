...
public interface SqlBuilder {
    ...
    static DeleteDSL<DeleteModel> deleteFrom(Class<?> entityClass) {
        return DeleteDSL.deleteFrom(entityClass);
    }
    ...
    /**
     * Insert a Batch of records. The model object is structured to support bulk inserts
     * with JDBC batch support.
    ...
    /**
     * Insert multiple records in a single statement. The model object is structured as a
     * single insert statement with multiple values clauses. This statement is suitable
     * for use with a small number of records. It is not suitable for large bulk inserts
     * as it is possible to exceed the limit of parameter markers in a prepared statement.
     *
     * <p>
    ...
    static UpdateDSL<UpdateModel> update(Class<?> entityClass) {
        return UpdateDSL.update(entityClass);
    }
    ...
    static <T> CriteriaGroup group(BindableColumn<T> column, VisitableCondition<T> condition,
        ...
        return new CriteriaGroup.Builder().withInitialCriterion(
                new ColumnAndConditionCriterion.Builder<T>().withColumn(column).withCondition(condition).build())
                .withSubCriteria(subCriteria).build();
    }
    ...
    static CriteriaGroup group(List<AndOrCriteriaGroup> subCriteria) {
        return new CriteriaGroup.Builder().withSubCriteria(subCriteria).build();
    }
    ...
    static <T> NotCriterion not(BindableColumn<T> column, VisitableCondition<T> condition,
            List<AndOrCriteriaGroup> subCriteria) {
        return new NotCriterion.Builder().withInitialCriterion(
                new ColumnAndConditionCriterion.Builder<T>().withColumn(column).withCondition(condition).build())
                .withSubCriteria(subCriteria).build();
    }
    ...
    static NotCriterion not(SqlCriterion initialCriterion, List<AndOrCriteriaGroup> subCriteria) {
        return new NotCriterion.Builder().withInitialCriterion(initialCriterion).withSubCriteria(subCriteria).build();
    }

    static NotCriterion not(List<AndOrCriteriaGroup> subCriteria) {
        return new NotCriterion.Builder().withSubCriteria(subCriteria).build();
    }
    ...
    static <T> AndOrCriteriaGroup or(BindableColumn<T> column, VisitableCondition<T> condition,
        ...
        return new AndOrCriteriaGroup.Builder()
                ...
                .withSubCriteria(Arrays.asList(subCriteria)).build();
    }
    ...
    static AndOrCriteriaGroup or(SqlCriterion initialCriterion, AndOrCriteriaGroup... subCriteria) {
        return new AndOrCriteriaGroup.Builder().withConnector("or") //$NON-NLS-1$
                .withInitialCriterion(initialCriterion).withSubCriteria(Arrays.asList(subCriteria)).build();
    }
    ...
    static AndOrCriteriaGroup and(SqlCriterion initialCriterion, AndOrCriteriaGroup... subCriteria) {
        return new AndOrCriteriaGroup.Builder().withConnector("and") //$NON-NLS-1$
                .withInitialCriterion(initialCriterion).withSubCriteria(Arrays.asList(subCriteria)).build();
    }
    ...
    static JoinCriterion and(BasicColumn joinColumn, JoinCondition joinCondition) {
        return new JoinCriterion.Builder().withConnector("and") //$NON-NLS-1$
                .withJoinColumn(joinColumn).withJoinCondition(joinCondition).build();
    }

    static JoinCriterion on(BasicColumn joinColumn, JoinCondition joinCondition) {
        return new JoinCriterion.Builder().withConnector("on") //$NON-NLS-1$
                .withJoinColumn(joinColumn).withJoinCondition(joinCondition).build();
    }
    ...
    /**
     * Creates a sort specification based on a String. This is useful when a column has
     * been aliased in the select list. For example:
     *
     * <pre>
     *     select(foo.as("bar"))
     *     .from(baz)
    ...
    /**
     * Creates a sort specification based on a column and a table alias. This can be
     * useful in a join where the desired sort order is based on a column not in the
     * select list. This will likely fail in union queries depending on database support.
    ...
    class InsertIntoNextStep {
        ...
        public InsertSelectDSL withSelectStatement(Buildable<SelectModel> selectModelBuilder) {
            return InsertSelectDSL.insertInto(table).withSelectStatement(selectModelBuilder);
        }

        public InsertSelectDSL.SelectGatherer withColumnList(SqlColumn<?>... columns) {
            return InsertSelectDSL.insertInto(table).withColumnList(columns);
        }
        ...
        public <T> GeneralInsertDSL.SetClauseFinisher<T> set(SqlColumn<T> column) {
            return GeneralInsertDSL.insertInto(table).set(column);
        }
