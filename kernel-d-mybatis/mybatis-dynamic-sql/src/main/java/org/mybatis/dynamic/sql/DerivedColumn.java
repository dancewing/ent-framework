 ...
 * {@link SqlColumn} is that this class does not have a related {@link SqlTable} and
 * therefore ignores any table qualifier set in a query. If a table qualifier is required
 * it can be set directly in the builder for this class.
 *
 * @param <T> The Java type that corresponds to this column - not used except for compiler
...
public class DerivedColumn<T> implements BindableColumn<T> {
    ...
    public DerivedColumn<T> as(String columnAlias) {
        return new Builder<T>().withName(name).withColumnAlias(columnAlias).withJdbcType(jdbcType)
                .withTypeHandler(typeHandler).withTableQualifier(tableQualifier).build();
    }

    public static <T> DerivedColumn<T> of(String name) {
        return new Builder<T>().withName(name).build();
    }

    public static <T> DerivedColumn<T> of(String name, String tableQualifier) {
        return new Builder<T>().withName(name).withTableQualifier(tableQualifier).build();
    }
