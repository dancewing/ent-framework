...
public abstract class AbstractTwoValueCondition<T> implements VisitableCondition<T> {
	...
	/**
	 * If renderable and the values match the predicate, returns this condition. Else
	 * returns a condition that will not render.
	 * @param predicate predicate applied to the values, if renderable
	 * @return this condition if renderable and the values match the predicate, otherwise
	 * a condition that will not render.
	 */
	public abstract AbstractTwoValueCondition<T> filter(BiPredicate<? super T, ? super T> predicate);

	/**
	 * If renderable and both values match the predicate, returns this condition. Else
	 * returns a condition that will not render. This function implements a
	 * short-circuiting test. If the first value does not match the predicate, then the
	 * second value will not be tested.
