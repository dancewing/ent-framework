...
public class BatchInsertRenderer<T> {
    ...
    public BatchInsert<T> render() {
        ...
        return BatchInsert.withRecords(model.records()).withInsertStatement(calculateInsertStatement(fieldsAndValues))
                .build();
    }
    ...
    private String calculateValuesPhrase(List<FieldAndValue> fieldsAndValues) {
        return fieldsAndValues.stream().map(FieldAndValue::valuePhrase)
