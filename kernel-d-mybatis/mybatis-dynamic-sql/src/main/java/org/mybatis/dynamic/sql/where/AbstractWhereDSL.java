...
public abstract class AbstractWhereDSL<T extends AbstractWhereDSL<T>> implements ConfigurableStatement<T> {
    ...
    public <S> T where(BindableColumn<S> column, VisitableCondition<S> condition,
        ...
        initialCriterion = ColumnAndConditionCriterion.withColumn(column).withCondition(condition)
    ...
    public T where(ExistsPredicate existsPredicate, List<AndOrCriteriaGroup> subCriteria) {
        initialCriterion = new ExistsCriterion.Builder().withExistsPredicate(existsPredicate)
                .withSubCriteria(subCriteria).build();
        return getThis();
    }
    ...
    public T where(SqlCriterion initialCriterion, List<AndOrCriteriaGroup> subCriteria) {
        this.initialCriterion = new CriteriaGroup.Builder().withInitialCriterion(initialCriterion)
                .withSubCriteria(subCriteria).build();
        return getThis();
    }
    ...
    public T where(List<AndOrCriteriaGroup> criteria) {
        initialCriterion = new CriteriaGroup.Builder().withSubCriteria(criteria).build();
        return getThis();
    }
    ...
    private void addSubCriteria(String connector, SqlCriterion initialCriterion, List<AndOrCriteriaGroup> subCriteria) {
        this.subCriteria.add(new AndOrCriteriaGroup.Builder().withInitialCriterion(initialCriterion)
                .withConnector(connector).withSubCriteria(subCriteria).build());
    }

    private void addSubCriteria(String connector, List<AndOrCriteriaGroup> criteria) {
        this.subCriteria
                .add(new AndOrCriteriaGroup.Builder().withConnector(connector).withSubCriteria(criteria).build());
    }
