...
public interface AnimalDataMapper extends CommonDeleteMapper, GenericInsertMapper<AnimalData>, CommonUpdateMapper {
	...
	@Select({ "select id, animal_name, brain_weight, body_weight", "from AnimalData", "${whereClause}" })
	@ResultMap("AnimalDataResult")
	List<AnimalData> selectWithWhereClause(WhereClauseProvider whereClause);

	@Select({ "select id, animal_name, brain_weight, body_weight", "from AnimalData", "${whereClause}" })
	@ResultMap("AnimalDataResult")
	Optional<AnimalData> selectOneAnimalData(WhereClauseProvider whereClause);

	@Select({ "select a.id, a.animal_name, a.brain_weight, a.body_weight", "from AnimalData a", "${whereClause}" })
	@ResultMap("AnimalDataResult")
	List<AnimalData> selectWithWhereClauseAndAlias(WhereClauseProvider whereClause);

	@Select({ "select id, animal_name, brain_weight, body_weight", "from AnimalData",
			"${whereClauseProvider.whereClause}", "order by id",
	...
	@Select({ "select b.id, b.animal_name, b.brain_weight, b.body_weight", "from AnimalData b",
			"${whereClauseProvider.whereClause}", "order by id",
			"OFFSET #{offset,jdbcType=INTEGER} LIMIT #{limit,jdbcType=INTEGER}" })
	@ResultMap("AnimalDataResult")
	List<AnimalData> selectWithWhereClauseAliasLimitAndOffset(
