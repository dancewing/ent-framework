 ...
 ******************************************************************************/
package io.entframework.kernel.sms.aliyun;

import cn.hutool.core.text.CharSequenceUtil;
import cn.hutool.core.util.ObjectUtil;
import io.entframework.kernel.sms.aliyun.enums.AliyunSmsResultEnum;
import io.entframework.kernel.sms.aliyun.msign.MultiSignManager;
...
public class AliyunSmsSender implements SmsSenderApi {
    ...
    private IAcsClient initClient() {
        ...
        DefaultProfile profile = DefaultProfile.getProfile(aliyunSmsProperties.getRegionId(), accessKeyId,
                accessKeySecret);
     ...
     */
    private JSONObject createSmsRequest(String phoneNumber, String templateCode, Map<String, Object> params,
    ...
    private void assertSendSmsParams(String phoneNumber, String templateCode, Map<String, Object> params,
        ...
        if (CharSequenceUtil.isBlank(phoneNumber)) {
            throw new SmsException(SmsExceptionEnum.SEND_SMS_PARAM_NULL, "电话号码");
        }

        if (CharSequenceUtil.isBlank(templateCode)) {
            throw new SmsException(SmsExceptionEnum.SEND_SMS_PARAM_NULL, "模板号templateCode");
        }

        if (ObjectUtil.isEmpty(params)) {
            throw new SmsException(SmsExceptionEnum.SEND_SMS_PARAM_NULL, "模板参数");
        }

        if (ObjectUtil.isEmpty(aliyunSmsProperties)) {
            throw new SmsException(SmsExceptionEnum.SEND_SMS_PARAM_NULL, "短信配置properties");
        }
    }
