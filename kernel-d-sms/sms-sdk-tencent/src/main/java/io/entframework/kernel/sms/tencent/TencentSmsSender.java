        ...
        try {
            ...
            if (sendStatusSet != null && sendStatusSet.length > 0) {
                if (!"Ok".equals(sendStatusSet[0].getCode())) {
                    throw new SmsException(SmsExceptionEnum.TENCENT_SMS_PARAM_NULL, sendStatusSet[0].getCode(), sendStatusSet[0].getMessage());
                }
            }
        } catch (TencentCloudSDKException e) {
            throw new SmsException(SmsExceptionEnum.TENCENT_SMS_PARAM_NULL, SmsExceptionEnum.TENCENT_SMS_PARAM_NULL.getErrorCode(), e.getMessage());
        }
    }
}
