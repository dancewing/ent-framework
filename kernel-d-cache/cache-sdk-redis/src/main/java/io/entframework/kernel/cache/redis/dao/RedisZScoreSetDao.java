...
public interface RedisZScoreSetDao extends RedisBaseDao {
    ...
    /**
     * 按顺序/倒序获取指定redis zset中，分数值从startScore到endScore之间的数据元素集合(仅包括数据)，返回结果中指定offset和limit<br/>
     * startScoreInclusive和endScoreInclusive分别用于指定：返回结果中是否包含初始和结束的score值
     *
     * @param key                 redisKey值
     * @param startScore          开始位置的score值
    ...
    /**
     * 按顺序/倒序获取指定redis zset中，分数值从startScore到endScore之间的实体元素集合(包括数据和分值)，返回结果中指定offset和limit<br/>
     * startScoreInclusive和endScoreInclusive分别用于指定：返回结果中是否包含初始和结束的score值
     *
     * @param key                 redisKey值
     * @param startScore          开始位置的score值
     ...
     * @return 在指定范围内的redis zset实体元素集合
     */
    <V> Collection<DistributedScoredEntry<V>> entryRange(String key, double startScore, boolean startScoreInclusive, double endScore,
                                                         boolean endScoreInclusive, int offset, int limit, boolean reverse);
