...
public class RedisCacheManager<V> extends AbstractRedisCacheManager<V> {
    ...
    @Override
    public Collection<DistributedScoredEntry<V>> getZSetEntryRange(String key, int fromIndex, int toIndex,
            boolean reverse) {
        String redisKey = this.getRedisKey(key);
    ...
    @Override
    public Collection<DistributedScoredEntry<V>> getZSetEntryRange(String key, double startScore,
            boolean startScoreInclusive, double endScore, boolean endScoreInclusive, boolean reverse) {
        String redisKey = this.getRedisKey(key);
        return redisZScoreSetDao.entryRange(redisKey, startScore, startScoreInclusive, endScore, endScoreInclusive,
                reverse);
    }
