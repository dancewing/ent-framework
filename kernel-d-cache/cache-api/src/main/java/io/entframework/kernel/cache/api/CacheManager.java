...
public interface CacheManager<V> {
    ...
    /**
     * 获取指定key的ttl(秒)， 返回值说明 -1: 未设置失效时间; -2: key不存在; 其他: 失效时间
     * @param key cache key值
    ...
    /**
     * 设置普通数据到cache中 在后续不设置过期时间的情况下，该数据将永不过期
     * @param key cache key值
    ...
    /**
     * 批量设置普通数据到cache中 在后续不设置过期时间的情况下，这些数据将永不过期
     * @param values 待批量插入到数据库中的数据
     */
    void set(Map<String, V> values);

    /**
     * 设置普通数据到cache中 带有指定的存活时间（时间单位：毫秒）
     * @param key cache key值
     * @param value 存储到cache中的数据
     * @param timeToLive 数据存活时间，时间单位：毫秒
     */
    ...
    /**
     * 批量设置普通数据到cache中 带有指定的存活时间（时间单位：毫秒）
     * @param values 待批量插入到数据库中的数据
     * @param timeToLive 数据存活时间，时间单位：毫秒
     */
    ...
    /**
     * 设置指定的key存放为null值，一般用于防止缓存穿透。不抛出cache操作相关的异常 数据具有指定的存活时间（时间单位：毫秒）
     * @param key cache key值
     * @param timeToLive 数据存活时间
     */
    ...
    /**
     * 尝试设置普通数据到cache中，如果数据已存在，则返回false 在后续不设置过期时间的情况下，该数据将永不过期
     * @param key cache key值
     * @param value 存储到cache中的数据
    ...
    /**
     * 尝试设置普通数据到cache中，如果数据已存在，则返回false 带有指定的存活时间（时间单位：毫秒）
     * @param key cache key值
     * @param value 存储到cache中的数据
     * @param timeToLive 数据存活时间，时间单位：毫秒
     * @return 当key值所对应的数据不存在，则设置成功时，返回true
    ...
    /**
     * 尝试设置普通数据到cache中，如果数据已存在，则返回false 带有指定的存活时间
     * @param key cache key值
     * @param value 存储到cache中的数据
     * @param timeToLive 数据存活时间，时间单位：毫秒
     * @param timeUnit 数据存活时间timeToLive的时间单位
    ...
    /**
     * 将cache map中的数据copy到另外一个map结构中 1、如果fromKey不存在，则返回false
     * 2、如果fromKey不是map结构，则抛出cache的操作异常 3、如果toKey已存在，但不是map结构，则抛出cache的操作异常
