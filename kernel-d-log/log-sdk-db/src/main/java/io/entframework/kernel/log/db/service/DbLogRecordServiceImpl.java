...
public class DbLogRecordServiceImpl implements LogRecordApi {
    ...
    public boolean addBatch(List<LogRecordDTO> logRecords) {
        ...
        List<SysLogRequest> sysLogList = logRecords.stream().map(logRecordDTO -> {
            SysLogRequest sysLog = new SysLogRequest();
            // 复制logRecordDTO对象属性到sysLog
            BeanUtil.copyProperties(logRecordDTO, sysLog);

            // 日志名称为空的话则获取默认日志名称
            if (CharSequenceUtil.isEmpty(sysLog.getLogName())) {
                sysLog.setLogName(LogConstants.LOG_DEFAULT_NAME);
            }

            // 服务名称为空的话则获取默认服务名称
            if (CharSequenceUtil.isEmpty(sysLog.getAppName())) {
        ...
        // 插入到数据库
        return !sysLogService.insertMultiple(sysLogList).isEmpty();
    }
