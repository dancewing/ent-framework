...
public class SysThemeTemplateFieldServiceImpl
        extends BaseServiceImpl<SysThemeTemplateFieldRequest, SysThemeTemplateFieldResponse, SysThemeTemplateField>
    ...
    public void add(SysThemeTemplateFieldRequest sysThemeTemplateFieldRequest) {
        ...
        sysThemeTemplateField.setFieldRequired(ObjectUtil.isNull(sysThemeTemplateFieldRequest.getFieldType())
                ? YesOrNotEnum.N : sysThemeTemplateFieldRequest.getFieldRequired());
    ...
    public void del(SysThemeTemplateFieldRequest sysThemeTemplateFieldRequest) {
        ...
        if (sysThemeTemplateField.getFieldCode().toUpperCase(Locale.ROOT)
    ...
    public SysThemeTemplateFieldResponse detail(SysThemeTemplateFieldRequest sysThemeTemplateFieldRequest) {
        return this.converterService.convert(this.queryThemeTemplateFieldById(sysThemeTemplateFieldRequest),
                getResponseClass());
    }

    @Override
    public PageResult<SysThemeTemplateFieldResponse> findPage(
            SysThemeTemplateFieldRequest sysThemeTemplateFieldRequest) {
    ...
    public List<SysThemeTemplateFieldResponse> findRelList(SysThemeTemplateFieldRequest sysThemeTemplateFieldRequest) {
        ...
        if (!fieldCodes.isEmpty()) {
            sysThemeTemplateFields = getRepository().select(SysThemeTemplateField.class,
                    c -> c.where(SysThemeTemplateFieldDynamicSqlSupport.fieldCode, SqlBuilder.isIn(fieldCodes)));
        }
        ...
        return sysThemeTemplateFields.stream()
                .map(sysThemeTemplateField -> this.converterService.convert(sysThemeTemplateField, getResponseClass()))
                .toList();
    }
    ...
    @Override
    public List<SysThemeTemplateFieldResponse> findNotRelList(
            SysThemeTemplateFieldRequest sysThemeTemplateFieldRequest) {
        // 查询有关联的属性
        ...
        if (fieldCodes.size() > 0) {
            sysThemeTemplateFields = getRepository().select(getEntityClass(),
                    c -> c.where(SysThemeTemplateFieldDynamicSqlSupport.fieldCode, SqlBuilder.isNotIn(fieldCodes)));
        }
        else {
     ...
     */
    private SysThemeTemplateField queryThemeTemplateFieldById(
            SysThemeTemplateFieldRequest sysThemeTemplateFieldRequest) {
        Optional<SysThemeTemplateField> sysThemeTemplateField = this.getRepository()
                .selectByPrimaryKey(getEntityClass(), sysThemeTemplateFieldRequest.getFieldId());
