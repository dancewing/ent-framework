...
public class DataScopeService implements DataScopeApi {
	...
	public DataScopeDTO getDataScope(Long userId, List<SysRoleResponse> sysRoles) {
		...
		// 获取角色中的数据范围类型
		Set<DataScopeTypeEnum> dataScopeTypeEnums = sysRoles.stream().map(SysRoleResponse::getDataScopeTypeEnum)
		...
		if (dataScopeTypeEnums.contains(DataScopeTypeEnum.DEPT_WITH_CHILD)) {
			...
			Set<Long> subOrgIds = dbOperatorApi.findSubListByParentId("hr_organization", "org_pids", "org_id",
					organizationId);
