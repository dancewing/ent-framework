...
public class MenusFactory {
     ...
     * @date 2021/1/7 18:17
     */
    public static List<AntdMenuDTO> createTotalMenus(Map<Long, List<SysMenuResponse>> appSortedMenus, Long activeAppId, boolean createTop) {
        ...
        if (appSortedMenus.containsKey(activeAppId)) {
            ...
            List<SysMenuResponse> treeStructMenu = new DefaultTreeBuildFactory<SysMenuResponse>(TreeConstants.DEFAULT_PARENT_ID.toString()).doTreeBuild(appSortedMenus.get(activeAppId));
        ...
        for (Map.Entry<Long, List<SysMenuResponse>> entry : appSortedMenus.entrySet()) {
            if (!entry.getKey().equals(activeAppId)) {
                ...
                List<SysMenuResponse> treeStructMenu = new DefaultTreeBuildFactory<SysMenuResponse>(TreeConstants.DEFAULT_PARENT_ID.toString()).doTreeBuild(entry.getValue());
                List<AntdMenuDTO> antdMenuDTOS = doModelTransfer(treeStructMenu);
     ...
     */
    public static List<MenuTreeResponse> parseMenuAndButtonTreeResponse(List<SysMenu> sysMenuList, List<SysRoleMenuResponse> roleBindMenus) {
