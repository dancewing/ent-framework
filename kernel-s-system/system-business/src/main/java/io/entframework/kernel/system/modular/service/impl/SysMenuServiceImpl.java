...
@Slf4j
public class SysMenuServiceImpl extends BaseServiceImpl<SysMenuRequest, SysMenuResponse, SysMenu>
	...
	public void del(SysMenuRequest sysMenuRequest) {
		...
		// 逻辑删除，设置删除标识
		this.getRepository().update(getEntityClass(), c -> c.set(SysMenuDynamicSqlSupport.delFlag)
				.equalTo(YesOrNotEnum.Y).where(SysMenuDynamicSqlSupport.menuId, SqlBuilder.isIn(childIdList)));
	...
	public SysMenuResponse update(SysMenuRequest sysMenuRequest) {
		...
		if (sysMenuRequest.getMenuParentId() != null && sysMenuRequest.getMenuParentId() > 0
				&& sysMenuRequest.getAppId() == null) {
	...
	public List<MenuTreeResponse> getRoleMenuAndButtons(SysRoleRequest sysRoleRequest) {
		...
		// 查询所有菜单列表
		List<SysMenu> sysMenuList = this.getRepository().select(getEntityClass(),
				c -> c.where(SysMenuDynamicSqlSupport.delFlag, SqlBuilder.isEqualTo(YesOrNotEnum.N))
						.and(SysMenuDynamicSqlSupport.statusFlag, SqlBuilder.isEqualTo(StatusEnum.ENABLE))
						.and(SysMenuDynamicSqlSupport.menuId,
								SqlBuilder.isIn(menuWhereIdList).filter(ObjectUtil::isNotEmpty)));

		// 获取菜单的id集合
		// List<Long> menuIdList =
		// sysMenuList.stream().map(SysMenu::getMenuId).collect(Collectors.toList());

		// 获取角色绑定的菜单的id
		List<SysRoleMenuResponse> roleMenuList = roleServiceApi
				.getRoleMenuList(Collections.singletonList(sysRoleRequest.getRoleId()));
	...
	public List<SysMenuResponse> getCurrentUserMenus(String appCode) {
		...
		SelectDSLCompleter completer = c -> c
				...
				// .and(SysMenuDynamicSqlSupport.appId,
		...
		completer = c -> c.where(SysMenuDynamicSqlSupport.statusFlag, SqlBuilder.isEqualTo(StatusEnum.ENABLE))
				.and(SysMenuDynamicSqlSupport.delFlag, SqlBuilder.isEqualTo(YesOrNotEnum.N))
	...
	private SysMenu querySysMenu(SysMenuRequest sysMenuRequest) {
		Optional<SysMenu> sysMenuOptional = this.getRepository().selectByPrimaryKey(getEntityClass(),
				sysMenuRequest.getMenuId());
	...
	private String updateChildrenAppAndLevel(SysMenuRequest sysMenuRequest, SysMenu oldMenu) {
		...
		if (updateSubAppsFlag || updateSubPidsFlag) {
			...
			SelectDSLCompleter dslCompleter = c -> c.where(SysMenuDynamicSqlSupport.menuPids,
					SqlBuilder.isLike(String.valueOf(oldMenu.getMenuId())));
			...
			if (ObjectUtil.isNotEmpty(list)) {
				...
				if (updateSubPidsFlag) {
					list.forEach(child -> {
						...
						String oldParentCodesPrefix = oldPids + SymbolConstant.LEFT_SQUARE_BRACKETS
								+ oldMenu.getMenuId() + SymbolConstant.RIGHT_SQUARE_BRACKETS + SymbolConstant.COMMA;
	...
	public List<SysMenu> getMenuStatInfoByMenuIds(List<Long> menuIds) {
		...
		return this.getRepository().select(getEntityClass(),
				c -> c.where(SysMenuDynamicSqlSupport.menuId, SqlBuilder.isIn(menuIds))
						.orderBy(SysMenuDynamicSqlSupport.menuId.descending()));
	}
