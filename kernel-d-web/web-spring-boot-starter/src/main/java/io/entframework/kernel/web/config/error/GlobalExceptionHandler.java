...
package io.entframework.kernel.web.config.error;
...
import io.entframework.kernel.rule.util.ExceptionUtil;
import io.entframework.kernel.rule.util.HttpServletUtil;
import io.entframework.kernel.rule.util.ProjectUtil;
...
public class GlobalExceptionHandler {
	...
	public ErrorResponseData missingParam(
		...
		return renderJson(ValidatorExceptionEnum.MISSING_SERVLET_REQUEST_PARAMETER_EXCEPTION, parameterName,
				parameterType);
	}
	...
	public String authError(AuthException authException, HttpServletRequest request, HttpServletResponse response,
			Model model) {
		...
		if (AuthExceptionEnum.AUTH_EXPIRED_ERROR.getErrorCode().equals(errorCode)) {
			...
			if (HttpServletUtil.getNormalRequestFlag(request)) {
				...
				ErrorResponseData errorResponseData = renderJson(authException.getErrorCode(),
						authException.getUserTip(), authException);
				ResponseRenderUtil.renderJsonResponse(401, response, errorResponseData);
				return null;
			}
		}
		...
		// 默认响应前端json
		ErrorResponseData errorResponseData = renderJson(authException.getErrorCode(), authException.getUserTip(),
				authException);
		ResponseRenderUtil.renderJsonResponse(500, response, errorResponseData);
		return null;
	}
	...
	private ErrorResponseData renderJson(AbstractExceptionEnum exception, Object... params) {
		return renderJson(exception.getErrorCode(), CharSequenceUtil.format(exception.getUserTip(), params), null);
	}
	...
	private ErrorResponseData renderJson(Throwable throwable) {
		return renderJson(DefaultBusinessExceptionEnum.SYSTEM_RUNTIME_ERROR.getErrorCode(),
				DefaultBusinessExceptionEnum.SYSTEM_RUNTIME_ERROR.getUserTip(), throwable);
	}
	...
	private ErrorResponseData renderJson(String code, String message, Throwable throwable) {
		if (ObjectUtil.isNotNull(throwable)) {
			...
			ExceptionUtil.fillErrorResponseData(errorResponseData, throwable, null);
