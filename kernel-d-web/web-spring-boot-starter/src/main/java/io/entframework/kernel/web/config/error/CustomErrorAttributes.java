...
public class CustomErrorAttributes extends DefaultErrorAttributes {
	...
	public Map<String, Object> getErrorAttributes(WebRequest webRequest, ErrorAttributeOptions attributeOptions) {
		...
		if (throwable instanceof ServiceException serviceException) {
			return BeanUtil
					.beanToMap(new ErrorResponseData(serviceException.getErrorCode(), serviceException.getUserTip()));
		}

		// 3.如果返回的是404 http状态码
		Integer status = (Integer) defaultErrorAttributes.get("status");
		if (status.equals(HttpStatus.HTTP_NOT_FOUND)) {
			return BeanUtil.beanToMap(new ErrorResponseData(ValidatorExceptionEnum.NOT_FOUND.getErrorCode(),
					ValidatorExceptionEnum.NOT_FOUND.getUserTip()));
		}

		// 4.无法确定的返回服务器异常
		return BeanUtil
				.beanToMap(new ErrorResponseData(DefaultBusinessExceptionEnum.SYSTEM_RUNTIME_ERROR.getErrorCode(),
