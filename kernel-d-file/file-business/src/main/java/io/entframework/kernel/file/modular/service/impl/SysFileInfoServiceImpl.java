...
public class SysFileInfoServiceImpl extends BaseServiceImpl<SysFileInfoRequest, SysFileInfoResponse, SysFileInfo> implements SysFileInfoService {
    ...
    public SysFileInfoResponse uploadFile(MultipartFile file, SysFileInfoRequest sysFileInfoRequest) {
        ...
        if (YesOrNotEnum.Y == sysFileInfoRequest.getSecretFlag()) {
            fileAuthUrl = fileOperatorApi.getFileAuthUrl(sysFileInfo.getFileBucket(), sysFileInfo.getFileObjectName(), fileServerProperties.getFileTimeoutSeconds() * 1000,  loginUserApi.getToken());
        } else {
    ...
    public void download(SysFileInfoRequest sysFileInfoRequest, HttpServletResponse response) {
        ...
        if (YesOrNotEnum.Y == sysFileInfoResponse.getSecretFlag() && YesOrNotEnum.N == sysFileInfoRequest.getSecretFlag()) {
        ...
        try {
            for (int i = 0; i < fileInfoResponseList.size(); i++) {
                ...
                if (ObjectUtil.isNotEmpty(sysFileInfoResponse)) {
                    ...
                    // 判断公有文件下载时是否包含私有文件
                    if (secretFlag.equals(YesOrNotEnum.N.getValue()) && !secretFlag.equals(sysFileInfoResponse.getSecretFlag().getValue())) {
                        throw new FileException(FileExceptionEnum.SECRET_FLAG_INFO_ERROR, fileOriginName);
                    }

                    FileOperatorApi fileOperatorApi = fileOperatorFactory.getFileOperatorApi(sysFileInfoResponse.getFileLocation());
                    byte[] fileBytes = fileOperatorApi.getFileBytes(sysFileInfoResponse.getFileBucket(), sysFileInfoResponse.getFileObjectName());
    ...
    public SysFileInfoResponse versionBack(SysFileInfoRequest sysFileInfoRequest) {
        ...
        // 把之前的文件刷回
        UpdateDSLCompleter updateDSLCompleter = c ->
                c.set(SysFileInfoDynamicSqlSupport.fileStatus).equalTo(FileStatusEnum.OLD)
        ...
        // 修改文件状态
        updateDSLCompleter = c ->
                c.set(SysFileInfoDynamicSqlSupport.fileStatus).equalTo(FileStatusEnum.NEW)
    ...
    public String getFileAuthUrl(Long fileId, String token) {
        ...
        FileOperatorApi fileOperatorApi = fileOperatorFactory.getFileOperatorApi(sysFileInfo.getFileLocation());
        return fileOperatorApi.getFileAuthUrl(sysFileInfo.getFileBucket(), sysFileInfo.getFileObjectName(), fileServerProperties.getFileTimeoutSeconds(), token);
    }
