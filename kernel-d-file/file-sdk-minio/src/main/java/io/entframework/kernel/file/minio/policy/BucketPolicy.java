...
public class BucketPolicy {
	...
	private static final ObjectMapper objectMapper = new ObjectMapper()
			.enable(DeserializationFeature.ACCEPT_SINGLE_VALUE_AS_ARRAY).setSerializationInclusion(Include.NON_NULL);
	...
	/**
	 * Reads JSON from given {@link Reader} and returns new {@link BucketPolicy} of given
	 * bucket name.
	 */
	...
	/**
	 * Appends given statement into statement list to have unique statements. - If
	 * statement already exists in statement list, it ignores. - If statement exists with
	 * different conditions, they are merged. - Else the statement is appended to
	 * statement list.
	 */
	private void appendStatement(Statement statement) {
		for (Statement s : statements) {
			...
			if (s.resources().containsAll(statement.resources()) && s.effect().equals(statement.effect()) && aws != null
					&& aws.containsAll(statement.principal().aws()) && conditions != null
					&& conditions.equals(statement.conditions())) {
