...
public enum FileExceptionEnum implements AbstractExceptionEnum {
     ...
     * 文件存储类型没有找到实现
     */
    FILE_STORAGE_NOT_FOUND(RuleConstants.USER_OPERATION_ERROR_TYPE_CODE + FileConstants.FILE_EXCEPTION_STEP_CODE + "18", "文件存储类型{}没有找到对应的API实现"),

    /**
     * 文件存储类型不能为空
     */
    FILE_STORAGE_TYPE_NOT_NULL(RuleConstants.USER_OPERATION_ERROR_TYPE_CODE + FileConstants.FILE_EXCEPTION_STEP_CODE + "19", "API的文件存储类型不能为null");
    /**
     * 错误编码
     */
