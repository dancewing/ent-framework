 ...
 ******************************************************************************/
package io.entframework.kernel.dict.api.serializer;

import cn.hutool.core.text.CharSequenceUtil;
import io.entframework.kernel.dict.api.context.DictContext;
...
public final class DictValueSerializer extends StdScalarSerializer<Object> {
    ...
    public void serialize(Object value, JsonGenerator gen, SerializerProvider provider) throws IOException {
        ...
        // 如果为空或者没有分隔符返回空串
        if (CharSequenceUtil.isBlank(fieldValue) || !fieldValue.contains(SEPARATOR)) {
            fieldValue = CharSequenceUtil.EMPTY;
        ...
        // 字典code为空，返回空串
        if (CharSequenceUtil.isBlank(dictCode) || NULL_STR.equals(dictCode)) {
            fieldValue = CharSequenceUtil.EMPTY;
            gen.writeString(fieldValue);
            return;
        }
        ...
        if (dictCode.contains(VALUE_SEPARATOR)) {
            ...
            for (String singleDictCode : dictCodeArray) {
                ...
                if (CharSequenceUtil.isNotBlank(dictName)) {
                    dictNames.append(dictName).append(VALUE_SEPARATOR);
                }
            }
            fieldValue = CharSequenceUtil.removeSuffix(dictNames.toString(), VALUE_SEPARATOR);
            gen.writeString(fieldValue);
            return;
        }
