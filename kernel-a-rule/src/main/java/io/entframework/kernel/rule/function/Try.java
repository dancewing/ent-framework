package io.entframework.kernel.rule.function;
...
public interface Try<V> {
    ...
    /**
     * Call the supplied {@link Callable} and return a successful {@code Try} that
     * contains the returned value or, in case an exception was thrown, a failed
     * {@code Try} that contains the exception.
     * @param action the action to try; must not be {@code null}
     * @return a succeeded or failed {@code Try} depending on the outcome of the supplied
     ...
     * @return a failed {@code Try} that contains the supplied value; never {@code null}
     */
    ...
    /**
     * If this {@code Try} is a success, apply the supplied transformer to its value and
     * return a new successful or failed {@code Try} depending on the transformer's
     * outcome; if this {@code Try} is a failure, do nothing.
     * @param transformer the transformer to try; must not be {@code null}
     * @return a succeeded or failed {@code Try}; never {@code null}
     */
    <U> Try<U> andThenTry(Transformer<V, U> transformer);

    /**
     * If this {@code Try} is a success, apply the supplied function to its value and
     * return the resulting {@code Try}; if this {@code Try} is a failure, do nothing.
     * @param function the function to apply; must not be {@code null}
     * @return a succeeded or failed {@code Try}; never {@code null}
     */
    <U> Try<U> andThen(Function<V, Try<U>> function);

    /**
     * If this {@code Try} is a failure, call the supplied action and return a new
     * successful or failed {@code Try} depending on the action's outcome; if this
     * {@code Try} is a success, do nothing.
     * @param action the action to try; must not be {@code null}
     * @return a succeeded or failed {@code Try}; never {@code null}
     */
    Try<V> orElseTry(Callable<V> action);

    /**
     * If this {@code Try} is a failure, call the supplied supplier and return the
     * resulting {@code Try}; if this {@code Try} is a success, do nothing.
     * @param supplier the supplier to call; must not be {@code null}
     * @return a succeeded or failed {@code Try}; never {@code null}
     */
    Try<V> orElse(Supplier<Try<V>> supplier);

    /**
     * If this {@code Try} is a success, get the contained value; if this {@code Try} is a
    ...
    /**
     * If this {@code Try} is a success, get the contained value; if this {@code Try} is a
     * failure, call the supplied {@link Function} with the contained exception and throw
     * the resulting {@link Exception}.
     * @param exceptionTransformer the transformer to be called with the contained
    ...
    /**
     * If this {@code Try} is a success, call the supplied {@link Consumer} with the
     * contained value; otherwise, do nothing.
     * @param valueConsumer the consumer to be called with the contained value, if
     * available; must not be {@code null}
     * @return the same {@code Try} for method chaining
     */
    Try<V> ifSuccess(Consumer<V> valueConsumer);

    /**
     * If this {@code Try} is a failure, call the supplied {@link Consumer} with the
     * contained exception; otherwise, do nothing.
     * @param causeConsumer the consumer to be called with the contained exception, if
     * available; must not be {@code null}
     * @return the same {@code Try} for method chaining
     */
    Try<V> ifFailure(Consumer<Exception> causeConsumer);

    /**
     * If this {@code Try} is a failure, return an empty {@link Optional}; if this
     * {@code Try} is a success, wrap the contained value using
     * {@link Optional#ofNullable(Object)}.
     * @return an {@link Optional}; never {@code null} but potentially <em>empty</em>
     */
     ...
     *
     * <p>
     * The {@code Transformer} interface is similar to {@link Function}, except that a
     * {@code Transformer} may throw an exception.
     */
    @FunctionalInterface
