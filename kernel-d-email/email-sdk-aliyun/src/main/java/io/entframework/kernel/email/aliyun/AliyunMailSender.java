 ...
 ******************************************************************************/
package io.entframework.kernel.email.aliyun;
...
import io.entframework.kernel.email.api.pojo.aliyun.AliyunMailSenderProperties;
import io.entframework.kernel.email.api.pojo.aliyun.AliyunSendMailParam;
...
public class AliyunMailSender implements MailSenderApi {
    ...
    public void sendAliyunMail(AliyunSendMailParam aliyunSendMailParam) {
        ...
        // 如果调用成功，正常返回httpResponse；
        // 如果调用失败则抛出异常，需要在异常中捕获错误异常码；错误异常码请参考对应的API文档;
        try {
            acsClient.getAcsResponse(singleSendRequest);
        }
        catch (ClientException e) {
            String userTip = CharSequenceUtil.format(EmailExceptionEnum.ALIYUN_MAIL_SEND_ERROR.getUserTip(),
                    e.getErrCode());
            throw new MailException(EmailExceptionEnum.ALIYUN_MAIL_SEND_ERROR, userTip);
        }
    }
    ...
    private IAcsClient initClient() {
        ...
        // 装载配置
        IClientProfile profile = DefaultProfile.getProfile("cn-hangzhou", aliyunMailSenderProperties.getAccessKeyId(),
