...
public class JavaMailSender implements MailSenderApi {
	...
	public void sendMail(SendMailParam sendMailParam) {
		...
		// spring发送邮件
		MailUtil.send(this.getConfigAccountInfo(sendMailParam), sendMailParam.getTos(), sendMailParam.getCcsTos(),
				sendMailParam.getBccsTos(), sendMailParam.getTitle(), sendMailParam.getContent(),
				sendMailParam.getImageMap(), false, sendMailParam.getFiles());
	}
	...
	public void sendMailHtml(SendMailParam sendMailParam) {
		...
		MailUtil.send(this.getConfigAccountInfo(sendMailParam), sendMailParam.getTos(), sendMailParam.getCcsTos(),
				sendMailParam.getBccsTos(), sendMailParam.getTitle(), sendMailParam.getContent(),
				sendMailParam.getImageMap(), true, sendMailParam.getFiles());
	}
