 ...
 ******************************************************************************/
package io.entframework.kernel.pinyin;

import io.entframework.kernel.pinyin.api.PinYinApi;
import io.entframework.kernel.pinyin.api.constants.PinyinConstants;
import io.entframework.kernel.pinyin.api.exception.PinyinException;
import io.entframework.kernel.pinyin.api.exception.enums.PinyinExceptionEnum;
	...
	public String parsePinyinString(String chineseString) {
		...
		try {
			...
			for (char chineseWord : chineseWordsArray) {
				// 判断是否为汉字字符
				if (Character.toString(chineseWord).matches(PinyinConstants.CHINESE_WORDS_REGEX)) {
			...
			throw new PinyinException(PinyinExceptionEnum.PARSE_ERROR, e1.getMessage());
		}
	}
	...
	private static String getFirstLetters(String chineseString, HanyuPinyinCaseType caseType) {
		...
		try {
			for (char word : chinesWords) {
				...
				if (str.matches(PinyinConstants.CHINESE_WORDS_REGEX)) {
