...
public abstract class BaseServiceImpl<REQ extends BaseRequest, RES, ENTITY> implements BaseService<REQ, RES, ENTITY> {
    ...
    protected BaseServiceImpl(Class<? extends REQ> requestClass, Class<? extends RES> responseClass,
    ...
    public List<RES> list(REQ request) {
        ENTITY query = this.converterService.convert(request, entityClass);
        List<ENTITY> results = generalRepository.selectBy(query, request.getPageNo(), request.getPageSize(), true,
                getOrderBy(request));
        return results.stream().map(entity -> this.converterService.convert(entity, responseClass)).toList();
    }
    ...
    public PageResult<RES> page(REQ request) {
        ...
        List<ENTITY> rows = generalRepository.selectBy(query, request.getPageNo(), request.getPageSize(), true,
                getOrderBy(request));
    ...
    public List<RES> insertMultiple(List<REQ> requests) {
        ...
        List<ENTITY> records = requests.stream().map(request -> this.converterService.convert(request, entityClass))
                .toList();
