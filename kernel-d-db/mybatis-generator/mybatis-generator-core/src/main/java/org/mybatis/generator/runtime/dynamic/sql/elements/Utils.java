    ...
    public static Optional<IntrospectedColumn> getVersionColumn(IntrospectedTable introspectedTable) {
        ...
        if (StringUtils.isNotEmpty(versionColumn)) {
            return introspectedTable.getNonPrimaryKeyColumns().stream()
                    .filter(column -> StringUtils.equals(versionColumn, column.getActualColumnName()))
                    .findFirst();
        }
        return Optional.empty();
    }

    public static String getSetVersionValue(IntrospectedColumn column, String tableFieldName, MethodAndImports.Builder builder) {
