...
public class RecordableAutoFillInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (object instanceof UpdateStatementProvider usp) {
            ...
            EntityMeta entityMeta = Entities.fromMapper(ClassUtils.toClassConfident(mapperName));
    ...
    private void setMapValue(EntityMeta entityInfo, Map<String, Object> parameters, String sql) {
        try {
            ...
            if (entityInfo.hasVersion()) {
                Optional<FieldMeta> versionColumn = entityInfo.findVersionColumn();
                if (versionColumn.isPresent()) {
                    String versionColumnName = versionColumn.get().getColumn().name();
                    ...
                    if (index > 0) {
                        ...
                        if (parameters.containsKey(key)) {
                            ...
                            if (version != null) {
                                parameters.put(key, increaseVersionVal(version.getClass(), version));
                            }
                        } else {
    ...
    private int getIndex(ArrayList<UpdateSet> updateSets, String columnName) {
        ...
        for (UpdateSet updateSet : updateSets) {
            boolean matched = updateSet.getColumns().stream().anyMatch(column -> StringUtils.equals(columnName, column.getColumnName()));
            if (matched) {
                return index;
            }
            index++;
    ...
    private void setValue(Object object, boolean isUpdate) {
        ...
        if (!isUpdate) {
            ...
            if (version != null) {
                ReflectUtil.setFieldValue(object, version, getInitVersionVal(version.getType()));
            }
        }
    }
    ...
    private Object getInitVersionVal(Class<?> clazz) {
    ...
    private Object increaseVersionVal(Class<?> clazz, Object value) {
        if (long.class.equals(clazz) || Long.class.equals(clazz)) {
            return ((long) value) + 1;
        } else if (int.class.equals(clazz) || Integer.class.equals(clazz)) {
            return ((int) value) + 1;
