...
public class EnumTypeStatusPlugin extends AbstractServerPlugin {
        ...
        if (StringUtility.stringHasValue(autoScan) && !StringUtility.isTrue(autoScan)) {
            ...
            if (StringUtility.stringHasValue(enumColumns)) {
                ...
                for (String enumColumnsStr : enumColumnsStrs) {
                    IntrospectedColumn column = GeneratorUtils.safeGetIntrospectedColumnByColumn(introspectedTable, enumColumnsStr);
                    if (column != null) {
                        try {
    ...
    public boolean modelFieldGenerated(Field field, TopLevelClass topLevelClass, IntrospectedColumn introspectedColumn, IntrospectedTable introspectedTable, ModelClassType modelClassType) {
        Map<String, EnumInfo> enumColumnsMap = (Map<String, EnumInfo>) introspectedTable.getAttribute(Constants.TABLE_ENUM_COLUMN_ATTR);
        if (enumColumnsMap != null && enumColumnsMap.get(field.getName()) != null) {
            ...
            FullyQualifiedJavaType columnJavaType = new FullyQualifiedJavaType(topLevelClass.getType().getFullyQualifiedName() + "." + innerEnum.getType());
