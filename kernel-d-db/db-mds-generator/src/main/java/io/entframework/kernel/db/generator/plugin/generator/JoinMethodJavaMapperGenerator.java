...
public class JoinMethodJavaMapperGenerator extends AbstractJavaMapperMethodGenerator {
    ...
    public void addInterfaceElements(Interface interfaze) {
        ...
        FieldAndImports oneFieldAndImports = generateLeftJoinFieldForOneAndImports();
        if (oneFieldAndImports != null) {
            interfaze.addField(oneFieldAndImports.getField());
            interfaze.addImportedTypes(oneFieldAndImports.getImports());
        }
        FieldAndImports manyFieldAndImports = generateLeftJoinFieldForManyAndImports();
        if (manyFieldAndImports != null) {
            interfaze.addField(manyFieldAndImports.getField());
            interfaze.addImportedTypes(manyFieldAndImports.getImports());
        }
    ...
    private MethodAndImports generateJoinOneMethodAndImports() {
        ...
        GeneratorUtils.addComment(method, CommentHelper.INSTANCE.getComments("leftJoinSelectOne","Mapper", Maps.of("MapperName", recordType.getShortName()+"Mapper").build()));
    ...
    private MethodAndImports generateJoinManyMethodAndImports() {
        ...
        GeneratorUtils.addComment(method, CommentHelper.INSTANCE.getComments("leftJoinSelectMany", "Mapper", Maps.of("MapperName", recordType.getShortName()+"Mapper").build()));
    ...
    private MethodAndImports generateDefaultLeftJoinSelectMethodAndImports() {
        ...
        parameterType.addTypeArgument(new FullyQualifiedJavaType("org.mybatis.dynamic.sql.select.SelectModel"));
        imports.add(parameterType);
        ...
        imports.add(new FullyQualifiedJavaType(JoinDetail.class.getName()));

        Method method = new Method("leftJoinSelect");
        method.setDefault(true);
        method.addParameter(new Parameter(parameterType, "queryExpressionDSL"));
        method.setReturnType(returnType);

        GeneratorUtils.addComment(method, CommentHelper.INSTANCE.getComments("leftJoinSelect", "Mapper", "default"));
        ...
        for (Pair<String, JoinTarget> detail : joinEntry.getDetails()) {
            ...
            if (joinTarget.getType() == JoinTarget.JoinType.MORE) {
                continue;
            }
            IntrospectedColumn leftTableJoinColumn = GeneratorUtils.getIntrospectedColumnByColumn(
            ...
            IntrospectedColumn targetJoinColumn = GeneratorUtils.getIntrospectedColumnByColumn(rightJoinTable, joinTarget.getJoinColumn());
        ...
        method.addBodyLine("return MyBatis3CustomUtils.leftJoinSelectList(this::leftJoinSelectMany, leftJoinSelectMany, " +
                tableFieldName + ", joinDetails , queryExpressionDSL);");
    ...
    private MethodAndImports generateDefaultLeftJoinSelectOneMethodAndImports() {
        ...
        FullyQualifiedJavaType parameterType = new FullyQualifiedJavaType("org.mybatis.dynamic.sql.select.QueryExpressionDSL");
        parameterType.addTypeArgument(new FullyQualifiedJavaType("org.mybatis.dynamic.sql.select.SelectModel"));
        imports.add(parameterType);
        ...
        method.addParameter(new Parameter(parameterType, "queryExpressionDSL"));
        method.setReturnType(returnType);
        GeneratorUtils.addComment(method, CommentHelper.INSTANCE.getComments("leftJoinSelectOne", "Mapper", "default"));
        method.addBodyLine("List<JoinDetail> joinDetails = new ArrayList<>();");
        for (Pair<String, JoinTarget> detail : joinEntry.getDetails()) {
            String leftTableJoinColumnName = detail.getLeft();
            JoinTarget joinTarget = detail.getRight();
        ...
        method.addBodyLine("return MyBatis3CustomUtils.leftJoinSelectOne(this::leftJoinSelectOne, leftJoinSelectOne, " +
                tableFieldName + ", joinDetails , queryExpressionDSL);");
     ...
     * @return
     */
    private FieldAndImports generateLeftJoinFieldForOneAndImports() {
        Set<FullyQualifiedJavaType> imports = new HashSet<>();
        ...
        Field field = new Field("leftJoinSelectOne", fieldType);
    ...
    private FieldAndImports generateLeftJoinFieldForManyAndImports() {
        Set<FullyQualifiedJavaType> imports = new HashSet<>();

        FullyQualifiedJavaType fieldType = new FullyQualifiedJavaType("org.mybatis.dynamic.sql.BasicColumn[]");
        imports.add(fieldType);
        List<IntrospectedTable> joinTargetTables = joinEntry.getDetails().stream()
                .filter(pair -> pair.getRight().getType() != JoinTarget.JoinType.MORE)
                .map(
                target -> GeneratorUtils.getIntrospectedTable(context, target.getRight().getRightTable())
        ).collect(Collectors.toList());
        ...
        Field field = new Field("leftJoinSelectMany", fieldType);
        field.setInitializationString("BasicColumn.columnList("
                + fragmentGenerator.getSelectList() + ","
                + joinTargetTables.stream().map(this::getJoinTargetSelectList).collect(Collectors.joining(", "))
                + ")");
