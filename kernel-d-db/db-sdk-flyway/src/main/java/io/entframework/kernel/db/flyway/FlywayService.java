...
public class FlywayService {
    ...
    public void migrate(DataSourceProperties dataSourceProperties, FlywayProperties flywayProperties, List<ModularPlugin> plugins){
        ...
        for (ModularPlugin plugin: plugins) {
            ...
            if (CharSequenceUtil.isEmpty(moduleMeta.getFlywayLocations())
                ...
                throw new DaoException(FlywayExceptionEnum.PLUGIN_FLYWAY_CONFIG_ERROR, moduleMeta.getName() + " 的 flywayLocations或者flywayTableSuffix未设置");
            }
            if (checkList.contains(moduleMeta.getFlywayTableSuffix())) {
                throw new DaoException(FlywayExceptionEnum.PLUGIN_FLYWAY_CONFIG_ERROR, moduleMeta.getName() + " 的flywayTableSuffix重复");
            }
