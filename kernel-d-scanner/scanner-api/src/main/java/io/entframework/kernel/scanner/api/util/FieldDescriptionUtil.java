...
import io.entframework.kernel.scanner.api.factory.ClassDetailMetadataFactory;
	...
	private static void parsingAnnotation(Field declaredField, FieldMetadata fieldDescription) {
		...
		if (annotations != null && annotations.length > 0) {
			...
			for (Annotation annotation : annotations) {
				Class<?>[] validateGroupsClasses = ClassReflectUtil.invokeAnnotationMethodIgnoreError(annotation,
						"groups", Class[].class);
