...
public class ClassReflectUtil {
     ...
     * @date 2022/1/6 16:09
     **/
    private static void fieldClassParsing(Field declaredField, FieldMetadata fieldDescription,
        ...
        if (genericType instanceof ParameterizedType
                && ((ParameterizedType) genericType).getActualTypeArguments()[0] instanceof ParameterizedType type) {
            ...
            fieldDescription.setGenericFieldMetadataType(FieldMetadataTypeEnum.GENERIC.getCode());
            fieldDescription.getGenericFieldMetadata().iterator().next()
                    .setGenericFieldMetadata(Collections.singleton(fieldMetadataItem));
        }
     ...
     * @date 2022/1/10 9:46
     **/
    private static void fieldMetadataGenericFill(Set<FieldMetadata> genericFieldMetadataSet, Class<?> declaredFieldType,
        ...
        for (Field field : declaredFields) {
            ...
            // 字段带泛型
            if (type instanceof ParameterizedType fieldGenericType) {
    ...
    private static void parsingAnnotation(Field declaredField, FieldMetadata fieldDescription) {
        ...
        if (annotations != null && annotations.length > 0) {
            ...
            for (Annotation annotation : annotations) {
                Class<?>[] validateGroupsClasses = invokeAnnotationMethodIgnoreError(annotation, "groups",
                        Class[].class);
    ...
    public static <T> T invokeAnnotationMethodIgnoreError(Annotation apiResource, String methodName,
            Class<T> resultType) {
        try {
     ...
     */
    public static void addGroupValidateAnnotation(Annotation fieldAnnotation, Class<?> validateGroupsClass,
    ...
    private static boolean isPrimitive(Class<?> clazz) {
        ...
        if (!isPrimitive && ObjectUtil.isNotEmpty(packageObj)) {
            isPrimitive = "java.lang".equals(packageObj.getName()) || "java.util".equals(packageObj.getName())
                    || "java.math".equals(packageObj.getName());
        }
