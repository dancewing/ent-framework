...
package io.entframework.kernel.scanner.api.context;
...
public class MetadataContext {
     ...
     */
    private static final ConcurrentMap<String, Set<String>> META_DATA_CLASS_COUNT_CONTEXT = new ConcurrentHashMap<>();

    /**
     * 第一个key是唯一id，第二个key是当前context下处理的参数类型
     */
    private static final ConcurrentMap<String, ParamTypeEnum> META_DATA_PARAM_TYPE_CONTEXT = new ConcurrentHashMap<>();

    /**
     * 第一个key是唯一id，第二个key是当前context下处理的参数名称
     */
    private static final ConcurrentMap<String, String> META_DATA_PARAM_NAME_CONTEXT = new ConcurrentHashMap<>();
