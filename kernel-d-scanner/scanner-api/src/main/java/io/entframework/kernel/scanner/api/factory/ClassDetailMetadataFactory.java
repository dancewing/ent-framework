...
package io.entframework.kernel.scanner.api.factory;
...
import io.entframework.kernel.scanner.api.util.ClassDescriptionUtil;
import io.entframework.kernel.scanner.api.util.ClassTypeUtil;
import io.entframework.kernel.scanner.api.util.FieldDescriptionUtil;
	...
	public static Set<FieldMetadata> createFieldDetailMetadataSet(Type fieldType, String uuid) {
		...
		switch (classFieldType) {
			...
			case BASE_ARRAY:
				...
				FieldMetadata baseMetadata = ClassDescriptionUtil.createClassMetadata(baseArrayComponentType,
						FieldTypeEnum.BASIC, uuid);
			...
			case BASE_COLLECTION:
				...
				FieldMetadata collectionFieldMetadata = ClassDescriptionUtil.createClassMetadata(Object.class,
						FieldTypeEnum.OBJECT, uuid);
				fieldMetadata = new LinkedHashSet<>();
			...
			for (FieldMetadata fieldMetadata : fieldDetailMetadataSet) {
				...
				// 如果T在携带在一个实体类上，例如ResponseData<T>这种形式
					...
					if (log.isDebugEnabled()) {
						log.debug("type: {}, getFieldClassPath: {}, genericType: {}", fieldType.getTypeName(),
								fieldMetadata.getFieldClassPath(), genericType.getTypeName());
					}
					try {
						current = getEntityWithGenericFieldMetadataList(
								Class.forName(fieldMetadata.getFieldClassPath()), genericType, uuid);
					}
